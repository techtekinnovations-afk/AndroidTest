...
public class MotionLabel extends View implements FloatLayout {
    ...
    public void setGravity(int gravity) {
        ...
        if ((gravity & GravityCompat.RELATIVE_HORIZONTAL_GRAVITY_MASK) != (this.mGravity & GravityCompat.RELATIVE_HORIZONTAL_GRAVITY_MASK)) {
        }
    ...
    private float getHorizontalOffset() {
        return (((((Float.isNaN(this.mFloatWidth) ? (float) getMeasuredWidth() : this.mFloatWidth) - ((float) getPaddingLeft())) - ((float) getPaddingRight())) - (this.mPaint.measureText(this.mText, 0, this.mText.length()) * (Float.isNaN(this.mBaseTextSize) ? 1.0f : this.mTextSize / this.mBaseTextSize))) * (this.mTextPanX + 1.0f)) / 2.0f;
    }
    ...
    private void setTypefaceFromAttrs(String familyName, int typefaceIndex, int styleIndex) {
        ...
        if (familyName == null || (tf2 = Typeface.create(familyName, styleIndex)) == null) {
            ...
            if (styleIndex > 0) {
                ...
                setTypeface(tf);
                int need = (~(tf != null ? tf.getStyle() : 0)) & styleIndex;
                TextPaint textPaint = this.mPaint;
                if ((need & 1) != 0) {
                    z = true;
                }
