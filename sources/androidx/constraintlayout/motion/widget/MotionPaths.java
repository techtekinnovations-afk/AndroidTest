...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    public void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float position = ((float) keyPosition.mFramePosition) / 100.0f;
        this.mTime = position;
        this.mDrawPath = keyPosition.mDrawPath;
        float scaleWidth = Float.isNaN(keyPosition.mPercentWidth) ? position : keyPosition.mPercentWidth;
        float scaleHeight = Float.isNaN(keyPosition.mPercentHeight) ? position : keyPosition.mPercentHeight;
        ...
        float dxdx = Float.isNaN(keyPosition.mPercentX) ? position : keyPosition.mPercentX;
        float dxdy = 0.0f;
        float dydx = Float.isNaN(keyPosition.mAltPercentY) ? 0.0f : keyPosition.mAltPercentY;
        float dydy = Float.isNaN(keyPosition.mPercentY) ? position : keyPosition.mPercentY;
        if (!Float.isNaN(keyPosition.mAltPercentX)) {
            dxdy = keyPosition.mAltPercentX;
        }
        this.mMode = 0;
    ...
    /* access modifiers changed from: package-private */
    public void initAxis(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        KeyPosition keyPosition = c;
        MotionPaths motionPaths = startTimePoint;
        MotionPaths motionPaths2 = endTimePoint;
        float position = ((float) keyPosition.mFramePosition) / 100.0f;
    ...
    MotionPaths(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        if (startTimePoint.mAnimateRelativeTo != Key.UNSET) {
            ...
            MotionPaths endTimePoint2 = startTimePoint;
            KeyPosition keyPosition = c;
            int i = parentHeight;
            int parentHeight2 = parentWidth;
            return;
        }
        MotionPaths endTimePoint3 = endTimePoint;
        MotionPaths startTimePoint2 = startTimePoint;
        KeyPosition c2 = c;
        int parentHeight3 = parentHeight;
        int parentHeight4 = parentWidth;
        switch (c2.mPositionType) {
            case 1:
                initPath(c2, startTimePoint2, endTimePoint3);
                return;
            case 2:
                int parentWidth2 = parentHeight4;
                int parentHeight5 = parentHeight3;
                KeyPosition c3 = c2;
                MotionPaths startTimePoint3 = startTimePoint2;
                initScreen(parentWidth2, parentHeight5, c3, startTimePoint3, endTimePoint3);
                MotionPaths endTimePoint4 = startTimePoint3;
                KeyPosition keyPosition2 = c3;
                int i2 = parentHeight5;
                int parentHeight6 = parentWidth2;
                return;
            case 3:
                initAxis(c2, startTimePoint2, endTimePoint3);
                return;
            default:
                initCartesian(c2, startTimePoint2, endTimePoint3);
                return;
        }
    }
    ...
    public void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        if (!Float.isNaN(keyPosition.mPercentX)) {
            ...
            this.mX = (float) ((int) (((float) parentWidth2) * keyPosition.mPercentX));
        } else {
            parentWidth2 = parentWidth;
        }
        if (!Float.isNaN(keyPosition.mPercentY)) {
            int i = parentWidth2;
            this.mY = (float) ((int) (((float) (parentHeight - ((int) this.mHeight))) * keyPosition.mPercentY));
        } else {
            int i2 = parentHeight;
        }
    ...
    public void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float scaleY = motionPaths2.mHeight - motionPaths.mHeight;
        this.mPosition = this.mTime;
        float path = Float.isNaN(keyPosition.mPercentX) ? position : keyPosition.mPercentX;
        float startCenterX = motionPaths.mX + (motionPaths.mWidth / 2.0f);
        float startCenterY = motionPaths.mY + (motionPaths.mHeight / 2.0f);
        float pathVectorX = (motionPaths2.mX + (motionPaths2.mWidth / 2.0f)) - startCenterX;
        float pathVectorY = (motionPaths2.mY + (motionPaths2.mHeight / 2.0f)) - startCenterY;
    ...
    public void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        if (this.mRelativeToController != null) {
            ...
            float f5 = drx;
            float drx2 = dv_y;
            float dpos_x = (float) (((double) drx) + (Math.sin((double) angle) * ((double) dradius2)) + (((double) drx2) * Math.cos((double) angle) * ((double) radius)));
            float f6 = dv_width2;
            float pos_y2 = pos_y;
            float f7 = dradius2;
            double cos = ((double) dry) - (((double) dradius2) * Math.cos((double) angle));
            double sin = Math.sin((double) angle) * ((double) radius);
            float f8 = angle;
            float dpos_y = (float) (cos + (sin * ((double) drx2)));
            float dv_x3 = dpos_x;
            float dv_y2 = dpos_y;
            v_x2 = pos_x;
            float v_y7 = pos_y2;
            ...
            if (!Float.isNaN(path_rotate)) {
                v_y = v_y7;
                float f11 = radius;
                view2.setRotation((float) (((double) path_rotate) + Math.toDegrees(Math.atan2((double) dv_y2, (double) dv_x3))));
            } else {
                v_y = v_y7;
                float f12 = radius;
            }
            ...
            c2 = 0;
            if (!Float.isNaN(path_rotate)) {
                view2.setRotation(0.0f + ((float) Math.toDegrees(Math.atan2((double) (dv_y3 + (dv_height2 / 2.0f)), (double) (dv_x4 + (dv_width3 / 2.0f))))) + path_rotate);
            }
            dv_x = v_y6;
            float f13 = dv_y3;
            float f14 = dv_x4;
        ...
        if (view2 instanceof FloatLayout) {
            ((FloatLayout) view2).layout(v_x2, dv_x, v_x2 + v_width, dv_x + v_height);
            return;
        }
        int l = (int) (v_x2 + 0.5f);
        int t = (int) (dv_x + 0.5f);
        ...
        int i_width = r - l;
        int i_height = b - t;
        if (i_width == view2.getMeasuredWidth() && i_height == view2.getMeasuredHeight()) {
            c = c2;
        }
