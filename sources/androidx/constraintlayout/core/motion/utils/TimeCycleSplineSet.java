...
public abstract class TimeCycleSplineSet {
    ...
    public float calcWave(float period) {
        ...
        switch (this.mWaveShape) {
            ...
            case 5:
                return (float) Math.cos((double) (sVal2PI * p));
            case 6:
                float x = 1.0f - Math.abs(((p * 4.0f) % 4.0f) - 2.0f);
                return 1.0f - (x * x);
            default:
                return (float) Math.sin((double) (sVal2PI * p));
        }
    }
    ...
    public void setPoint(int position, float value, float period, int shape, float offset) {
        ...
        this.mValues[this.mCount][0] = value;
        this.mValues[this.mCount][1] = period;
        this.mValues[this.mCount][2] = offset;
    ...
    public static class CustomSet extends TimeCycleSplineSet {
        ...
        public boolean setProperty(MotionWidget view, float t, long time, KeyCache cache) {
            ...
            if (Float.isNaN(this.mLastCycle)) {
                this.mLastCycle = cache.getFloatValue(motionWidget, this.mAttributeName, 0);
                if (Float.isNaN(this.mLastCycle)) {
    ...
    public void setup(int curveType) {
        ...
        Sort.doubleQuickSort(this.mTimePoints, this.mValues, 0, this.mCount - 1);
        int unique = 0;
        ...
        for (int i2 = 0; i2 < this.mCount; i2++) {
            if (i2 <= 0 || this.mTimePoints[i2] != this.mTimePoints[i2 - 1]) {
                ...
                values[k][0] = (double) this.mValues[i2][0];
                values[k][1] = (double) this.mValues[i2][1];
                values[k][2] = (double) this.mValues[i2][2];
                k++;
            }
        }
        this.mCurveFit = CurveFit.get(curveType, time, values);
