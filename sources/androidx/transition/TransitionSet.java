...
public class TransitionSet extends Transition {
    ...
    private int indexOfTransition(long playTime) {
        for (int i = 1; i < this.mTransitions.size(); i++) {
            if (this.mTransitions.get(i).mSeekOffsetInParent > playTime) {
                return i - 1;
            }
        }
    ...
    public void setCurrentPlayTimeMillis(long playTimeMillis, long lastPlayTimeMillis) {
        ...
        if (this.mParent != null) {
            ...
            if (j2 > duration && j3 > duration) {
                return;
            }
        }
        boolean isReverse = j2 < j3;
        if ((j2 >= 0 && j3 < 0) || (j2 <= duration && j3 > duration)) {
            this.mEnded = false;
            notifyListeners(Transition.TransitionNotification.ON_START, isReverse);
        }
        if (this.mPlayTogether) {
            for (int i = 0; i < this.mTransitions.size(); i++) {
                this.mTransitions.get(i).setCurrentPlayTimeMillis(j2, j3);
            }
            j = 0;
        } else {
            int startIndex = indexOfTransition(j3);
            if (j2 >= j3) {
                int i2 = startIndex;
                while (true) {
                    ...
                    j = j4;
                    long playTime = j2 - transitionStart;
                    if (playTime < j) {
                        break;
                    }
                    transition.setCurrentPlayTimeMillis(playTime, j3 - transitionStart);
                    i2++;
                    j4 = j;
                }
            } else {
                ...
                while (i4 >= 0) {
                    ...
                    long playTime2 = j2 - transitionStart2;
                    transition2.setCurrentPlayTimeMillis(playTime2, j3 - transitionStart2);
                    if (playTime2 >= 0) {
                        break;
                    }
                    i4--;
        ...
        if ((playTimeMillis > duration && j3 <= duration) || (playTimeMillis < j && j3 >= j)) {
