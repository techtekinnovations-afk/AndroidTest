...
public class CircularPropagation extends VisibilityPropagation {
    ...
    public long getStartDelay(ViewGroup sceneRoot, Transition transition, TransitionValues startValues, TransitionValues endValues) {
        ...
        if (epicenter != null) {
            ...
            int epicenterX2 = Math.round(((float) (loc[0] + (viewGroup.getWidth() / 2))) + viewGroup.getTranslationX());
            epicenterY = Math.round(((float) (loc[1] + (viewGroup.getHeight() / 2))) + viewGroup.getTranslationY());
            epicenterX = epicenterX2;
        }
        float distanceFraction = distance((float) viewCenterX, (float) viewCenterY, (float) epicenterX, (float) epicenterY) / distance(0.0f, 0.0f, (float) viewGroup.getWidth(), (float) viewGroup.getHeight());
