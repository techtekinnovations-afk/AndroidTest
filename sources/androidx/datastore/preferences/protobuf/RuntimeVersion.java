...
public final class RuntimeVersion {
    ...
    private static void validateProtobufGencodeVersionImpl(RuntimeDomain domain, int major, int minor, int patch, String suffix, String location) {
        if (!checkDisabled()) {
            ...
            if (major < 0 || minor < 0 || patch < 0) {
                ...
                if (major != 4) {
                    if (major == 3) {
                        logger.warning(String.format(" Protobuf gencode version %s is exactly one major version older than the runtime version %s at %s. Please update the gencode to avoid compatibility violations in the next runtime release.", new Object[]{gencodeVersionString, VERSION_STRING, location}));
                    } else {
                        throw new ProtobufRuntimeVersionException(String.format("Detected mismatched Protobuf Gencode/Runtime major versions when loading %s: gencode %s, runtime %s. Same major version is required.", new Object[]{location, gencodeVersionString, VERSION_STRING}));
                    }
                }
                if (28 < minor || (minor == 28 && 2 < patch)) {
                    throw new ProtobufRuntimeVersionException(String.format("Detected incompatible Protobuf Gencode/Runtime versions when loading %s: gencode %s, runtime %s. Runtime version cannot be older than the linked gencode version.", new Object[]{location, gencodeVersionString, VERSION_STRING}));
                }
                if (28 > minor || 2 > patch) {
                    logger.warning(String.format(" Protobuf gencode version %s is older than the runtime version %s at %s. Please avoid checked-in Protobuf gencode that can be obsolete.", new Object[]{gencodeVersionString, VERSION_STRING, location}));
                }
                if (!suffix.equals("")) {
                    throw new ProtobufRuntimeVersionException(String.format("Detected mismatched Protobuf Gencode/Runtime version suffixes when loading %s: gencode %s, runtime %s. Version suffixes must be the same.", new Object[]{location, gencodeVersionString, VERSION_STRING}));
                }
            } else {
    ...
    private static String versionString(int major, int minor, int patch, String suffix) {
        return String.format("%d.%d.%d%s", new Object[]{Integer.valueOf(major), Integer.valueOf(minor), Integer.valueOf(patch), suffix});
    }
