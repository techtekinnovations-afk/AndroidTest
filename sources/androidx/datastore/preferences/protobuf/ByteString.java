    ...
    public static final class Output extends OutputStream {
        ...
        private void flushLastBuffer() {
            if (this.bufferPos >= this.buffer.length) {
                ...
                this.flushedBuffers.add(new LiteralByteString(Arrays.copyOf(this.buffer, this.bufferPos)));
            }
    ...
    private static final class NioByteString extends LeafByteString {
        ...
        private ByteBuffer slice(int beginIndex, int endIndex) {
            if (beginIndex < this.buffer.position() || endIndex > this.buffer.limit() || beginIndex > endIndex) {
                throw new IllegalArgumentException(String.format("Invalid indices [%d, %d]", new Object[]{Integer.valueOf(beginIndex), Integer.valueOf(endIndex)}));
            }
            ByteBuffer slice = this.buffer.slice();
