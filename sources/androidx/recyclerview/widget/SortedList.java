...
public class SortedList<T> {
    ...
    private int add(T item, boolean notify) {
        T item2 = item;
        int index = findIndexOf(item2, this.mData, 0, this.mSize, 1);
        if (index == -1) {
            index = 0;
        } else if (index < this.mSize) {
            T existing = this.mData[index];
            if (this.mCallback.areItemsTheSame(existing, item2)) {
                if (this.mCallback.areContentsTheSame(existing, item2)) {
                    this.mData[index] = item2;
                    return index;
                }
                this.mData[index] = item2;
                this.mCallback.onChanged(index, 1, this.mCallback.getChangePayload(existing, item2));
                return index;
            }
        }
        addToData(index, item2);
        if (notify) {
    ...
    public int indexOf(T item) {
        if (this.mOldData != null) {
            T item2 = item;
            int index = findIndexOf(item2, this.mData, 0, this.mNewDataStart, 4);
            T item3 = item2;
            if (index != -1) {
                return index;
            }
            int index2 = findIndexOf(item3, this.mOldData, this.mOldDataStart, this.mOldDataSize, 4);
            if (index2 != -1) {
