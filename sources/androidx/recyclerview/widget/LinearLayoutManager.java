...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler, RecyclerView.State state, int startOffset, int endOffset) {
        ...
        if (!state2.willRunPredictiveAnimations() || getChildCount() == 0 || state2.isPreLayout()) {
            int i = startOffset;
            int i2 = endOffset;
    ...
    private boolean updateAnchorFromChildren(RecyclerView.Recycler recycler, RecyclerView.State state, AnchorInfo anchorInfo) {
        View referenceChild;
        boolean outOfBoundsAfter = false;
        if (getChildCount() == 0) {
            return false;
        }
        View focused = getFocusedChild();
        if (focused != null && anchorInfo.isViewValidAsAnchor(focused, state)) {
            ...
            return true;
        } else if (this.mLastStackFromEnd != this.mStackFromEnd || (referenceChild = findReferenceChild(recycler, state, anchorInfo.mLayoutFromEnd, this.mStackFromEnd)) == null) {
            return false;
        } else {
            anchorInfo.assignFromView(referenceChild, getPosition(referenceChild));
            if (!state.isPreLayout() && supportsPredictiveItemAnimations()) {
                int childStart = this.mOrientationHelper.getDecoratedStart(referenceChild);
                int childEnd = this.mOrientationHelper.getDecoratedEnd(referenceChild);
    ...
    public void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        if (this.mOrientation == 1) {
            if (isLayoutRTL()) {
                ...
                left2 = right - this.mOrientationHelper.getDecoratedMeasurementInOther(view);
            } else {
                left2 = getPaddingLeft();
                right = this.mOrientationHelper.getDecoratedMeasurementInOther(view) + left2;
            }
            if (layoutState.mLayoutDirection == -1) {
            ...
            if (layoutState.mLayoutDirection == -1) {
                ...
                bottom2 = layoutState.mOffset - result.mConsumed;
                left = i;
                bottom = bottom3;
            } else {
        ...
        layoutDecoratedWithMargins(view, bottom2, left, top, bottom);
    ...
    public View findReferenceChild(RecyclerView.Recycler recycler, RecyclerView.State state, boolean layoutFromEnd, boolean traverseChildrenInReverseOrder) {
        ...
        for (int i = start; i != end; i += diff) {
            ...
            int childStart = this.mOrientationHelper.getDecoratedStart(view);
            int childEnd = this.mOrientationHelper.getDecoratedEnd(view);
        ...
        if (bestSecondFind != null) {
            return bestSecondFind;
        }
        return bestFirstFind != null ? bestFirstFind : invalidMatch;
    }
