...
public class DiffUtil {
    ...
    private static Snake forward(Range range, Callback cb, CenteredArray forward, CenteredArray backward, int d) {
        ...
        int i = d;
        boolean checkForSnake = Math.abs(range2.oldSize() - range2.newSize()) % 2 == 1;
        int delta = range2.oldSize() - range2.newSize();
        for (int k = -i; k <= i; k += 2) {
    ...
    static class Diagonal {
        ...
        Diagonal(int x2, int y2, int size2) {
            this.x = x2;
            this.y = y2;
            this.size = size2;
        }
            ...
            while (diagonalIndex >= 0) {
                ...
                while (posY > endY) {
                    ...
                    if ((status2 & 12) != 0) {
                        ...
                        if (postponedUpdate2 == null) {
                            ...
                            batchingCallback.onMoved((currentListSize2 - postponedUpdate2.currentPos) - 1, posX);
                            if ((status2 & 4) != 0) {
                                batchingCallback.onChanged(posX, 1, this.mCallback.getChangePayload(oldPos, posY));
                            }
                        }
                    } else {
                ...
                for (int i2 = 0; i2 < diagonal.size; i2++) {
                    if ((this.mOldItemStatuses[posX2] & 15) == 2) {
                        batchingCallback.onChanged(posX2, 1, this.mCallback.getChangePayload(posX2, posY2));
                    }
                    posX2++;
                    posY2++;
                }
    ...
    static class CenteredArray {
        ...
        private final int mMid = (this.mData.length / 2);
