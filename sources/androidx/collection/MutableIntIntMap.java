...
public final class MutableIntIntMap extends IntIntMap {
                ...
                if ((((~$this$maskEmptyOrDeleted$iv$iv) << 7) & $this$maskEmptyOrDeleted$iv$iv & -9187201950435737472L) != -9187201950435737472L) {
                    ...
                    while (j$iv < bitCount$iv) {
                        if ((255 & slot$iv) < 128) {
                            ...
                            if (function2.invoke(Integer.valueOf(this.keys[index]), Integer.valueOf(this.values[index])).booleanValue()) {
                                removeValueAt(index);
                            }
                        } else {
                            $i$f$removeIf2 = $i$f$removeIf3;
    ...
    private final void resizeStorage(int newCapacity) {
        ...
        long[] previousMetadata2 = mutableIntIntMap.metadata;
        int[] previousKeys = mutableIntIntMap.keys;
        int[] previousValues = mutableIntIntMap.values;
        int previousCapacity = mutableIntIntMap._capacity;
        initializeStorage(newCapacity);
        int[] newKeys = mutableIntIntMap.keys;
        int[] newValues = mutableIntIntMap.values;
        int i = 0;
        while (i < previousCapacity) {
            if (((previousMetadata2[i >> 3] >> ((i & 7) << 3)) & 255) < 128) {
                ...
                int index = mutableIntIntMap.findFirstAvailableSlot(hash >>> 7);
