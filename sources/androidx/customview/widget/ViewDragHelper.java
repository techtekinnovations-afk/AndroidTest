...
public class ViewDragHelper {
    ...
    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
            ...
            this.mDefaultEdgeSize = (int) ((20.0f * context.getResources().getDisplayMetrics().density) + 0.5f);
    ...
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel) {
        ...
        int dy2 = dy;
        int dx2 = dx;
        int duration = computeSettleDuration(this.mCapturedView, dx2, dy2, xvel, yvel);
        this.mScroller.startScroll(startLeft, startTop, dx2, dy2, duration);
        setDragState(2);
        return true;
    }
    ...
    public boolean canScroll(View v, boolean checkV, int dx, int dy, int x, int y) {
        ...
        if (!checkV) {
            ...
            int i4 = dy;
            return true;
        } else if (v.canScrollVertically(-dy)) {
            return true;
        }
        return false;
    }
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        ...
        if (i != 0) {
            int clampedX3 = this.mCallback.clampViewPositionHorizontal(this.mCapturedView, left, i);
            ViewCompat.offsetLeftAndRight(this.mCapturedView, clampedX3 - oldLeft);
            clampedX = clampedX3;
        } else {
            int i3 = left;
            clampedX = clampedX2;
        ...
        if (i2 != 0) {
            int clampedY3 = this.mCallback.clampViewPositionVertical(this.mCapturedView, top, i2);
            ViewCompat.offsetTopAndBottom(this.mCapturedView, clampedY3 - oldTop);
            clampedY = clampedY3;
        } else {
            int i4 = top;
            clampedY = clampedY2;
