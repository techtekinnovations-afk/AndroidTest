...
public class FlexBuffersBuilder {
        ...
        this.keyComparator = new Comparator<Value>() {
            public int compare(Value o1, Value o2) {
                ...
                do {
                    ...
                    c2 = FlexBuffersBuilder.this.bb.get(io2);
                    if (c1 == 0) {
                        return c1 - c2;
                    }
                    ia++;
    ...
    public int endVector(String key, int start, boolean typed, boolean fixed) {
        int start2 = start;
        Value vec = createVector(putKey(key), start2, this.stack.size() - start, typed, fixed, (Value) null);
        while (this.stack.size() > start2) {
    ...
    private Value createVector(int key, int start, int length, boolean typed, boolean fixed, Value keys) {
        ...
        if (!fixed || typed) {
            int i4 = 0;
            int bitWidth = Math.max(0, widthUInBits((long) i3));
            int prefixElems = 1;
            if (value != null) {
                bitWidth = Math.max(bitWidth, value.elemWidth(this.bb.writePosition(), 0));
                prefixElems = 1 + 2;
            }
            int vectorType = 4;
            int bitWidth2 = bitWidth;
            for (int i5 = start; i5 < this.stack.size(); i5++) {
                bitWidth2 = Math.max(bitWidth2, this.stack.get(i5).elemWidth(this.bb.writePosition(), i5 + prefixElems));
                if (!typed) {
                    int i6 = start;
                } else if (i5 == start) {
                    vectorType = this.stack.get(i5).type;
                    if (!FlexBuffers.isTypedVectorElementType(vectorType)) {
                        throw new FlexBuffers.FlexBufferException("TypedVector does not support this element type");
                    }
                } else if (vectorType != this.stack.get(i5).type) {
                    throw new AssertionError();
                }
            }
            int i7 = start;
            if (!fixed || FlexBuffers.isTypedVectorElementType(vectorType)) {
                int byteWidth = align(bitWidth2);
                if (value != null) {
                    writeOffset(value.iValue, byteWidth);
                    writeInt(1 << value.minBitWidth, byteWidth);
                }
                if (!fixed) {
                    writeInt((long) i3, byteWidth);
                }
                int vloc = this.bb.writePosition();
                for (int i8 = start; i8 < this.stack.size(); i8++) {
                    writeAny(this.stack.get(i8), byteWidth);
                }
                if (!typed) {
                    for (int i9 = start; i9 < this.stack.size(); i9++) {
                        this.bb.put(this.stack.get(i9).storedPackedType(bitWidth2));
                    }
                }
                if (value != null) {
                    i = 9;
                    ...
                    if (typed) {
                        ...
                        i2 = FlexBuffers.toTypedVector(vectorType, i4);
                    } else {
                        i2 = 10;
                    }
                    i = i2;
                }
                return new Value(key, i, bitWidth2, (long) vloc);
            }
            throw new AssertionError();
        }
        throw new AssertionError();
    ...
    public int endMap(String key, int start) {
        ...
        int start2 = start;
        Value vec = createVector(iKey, start2, this.stack.size() - start, false, false, keys);
        while (this.stack.size() > start2) {
            this.stack.remove(this.stack.size() - 1);
        }
        this.stack.add(vec);
    ...
    private Value createKeyVector(int start, int length) {
        int i = length;
        int bitWidth = Math.max(0, widthUInBits((long) i));
        for (int i2 = start; i2 < this.stack.size(); i2++) {
            bitWidth = Math.max(bitWidth, Value.elemWidth(4, 0, (long) this.stack.get(i2).key, this.bb.writePosition(), i2 + 1));
        }
        int byteWidth = align(bitWidth);
        writeInt((long) i, byteWidth);
        int vloc = this.bb.writePosition();
        int i3 = start;
        while (i3 < this.stack.size()) {
            if (this.stack.get(i3).key != -1) {
                writeOffset((long) this.stack.get(i3).key, byteWidth);
                i3++;
            } else {
                throw new AssertionError();
            }
