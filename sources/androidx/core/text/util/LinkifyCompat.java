...
public final class LinkifyCompat {
    ...
    public static boolean addLinks(Spannable text, int mask) {
        ...
        if ((mask & 1) != 0) {
            text2 = text;
            gatherLinks(arrayList, text2, PatternsCompat.AUTOLINK_WEB_URL, new String[]{"http://", "https://", "rtsp://"}, Linkify.sUrlMatchFilter, (Linkify.TransformFilter) null);
        } else {
            text2 = text;
        }
        if ((mask & 2) != 0) {
            gatherLinks(arrayList, text2, PatternsCompat.AUTOLINK_EMAIL_ADDRESS, new String[]{MailTo.MAILTO_SCHEME}, (Linkify.MatchFilter) null, (Linkify.TransformFilter) null);
        }
        if ((mask & 8) != 0) {
            gatherMapLinks(arrayList, text2);
        }
        pruneOverlaps(arrayList, text2);
        if (arrayList.size() == 0) {
            return false;
        }
        ...
        while (it.hasNext()) {
            ...
            if (link.frameworkAddedSpan == null) {
                applyLink(link.url, link.start, link.end, text2);
            }
        }
        return true;
    }
    ...
    private static String makeUrl(String url, String[] prefixes, Matcher matcher, Linkify.TransformFilter filter) {
        String url2;
        if (filter != null) {
            url2 = filter.transformUrl(matcher, url);
        } else {
            url2 = url;
        }
        ...
        while (true) {
            ...
            if (url2.regionMatches(true, 0, prefix, 0, prefix.length())) {
                hasPrefix = true;
                if (!url2.regionMatches(false, 0, prefix, 0, prefix.length())) {
                    url2 = prefix + url2.substring(prefix.length());
                }
            } else {
                i++;
            }
