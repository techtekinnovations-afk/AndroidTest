...
public class TypefaceCompat {
    ...
    public static Typeface createFromResourcesFamilyXml(Context context, FontResourcesParserCompat.FamilyResourceEntry entry, Resources resources, int id, String path, int cookie, int style, ResourcesCompat.FontCallback fontCallback, Handler handler, boolean isRequestFromLayoutInflator) {
        ...
        if (entry instanceof FontResourcesParserCompat.ProviderResourceEntry) {
            ...
            typeface = FontsContractCompat.requestFont(context, providerEntry.getRequest(), style, isBlocking, timeout, ResourcesCompat.FontCallback.getHandler(handler2), new ResourcesCallbackAdapter(fontCallback2));
        } else {
    ...
    public static class ResourcesCallbackAdapter extends FontsContractCompat.FontRequestCallback {
        ...
        public void onTypefaceRetrieved(Typeface typeface) {
            if (this.mFontCallback != null) {
                this.mFontCallback.m1990lambda$callbackSuccessAsync$0$androidxcorecontentresResourcesCompat$FontCallback(typeface);
            }
        }

        public void onTypefaceRequestFailed(int reason) {
            if (this.mFontCallback != null) {
                this.mFontCallback.m1989lambda$callbackFailAsync$1$androidxcorecontentresResourcesCompat$FontCallback(reason);
            }
        }
    }
}
