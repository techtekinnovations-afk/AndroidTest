...
public final class WindowInsetsAnimationCompat {
    ...
    private static class Impl21 extends Impl {
        ...
        static Interpolator createInsetInterpolator(int animationMask, WindowInsetsCompat targetInsets, WindowInsetsCompat startingInsets) {
            if ((animationMask & 8) == 0) {
                return DEFAULT_INSET_INTERPOLATOR;
            }
            if (targetInsets.getInsets(WindowInsetsCompat.Type.ime()).bottom > startingInsets.getInsets(WindowInsetsCompat.Type.ime()).bottom) {
        ...
        private static class Impl21OnApplyWindowInsetsListener implements View.OnApplyWindowInsetsListener {
            ...
            public WindowInsets onApplyWindowInsets(final View v, WindowInsets insets) {
                ...
                if (!v.isLaidOut()) {
                    this.mLastInsets = WindowInsetsCompat.toWindowInsetsCompat(windowInsets, v);
                    return Impl21.forwardToViewIfNeeded(v, insets);
                }
                final WindowInsetsCompat targetInsets = WindowInsetsCompat.toWindowInsetsCompat(windowInsets, v);
                ...
                WindowInsetsAnimationCompat anim = new WindowInsetsAnimationCompat(animationMask, Impl21.createInsetInterpolator(animationMask, targetInsets, startingInsets), 160);
                ...
                Impl21.dispatchOnPrepare(v, anim, windowInsets, false);
                final WindowInsetsAnimationCompat anim2 = anim;
                final View view = v;
                AnonymousClass1 r0 = new ValueAnimator.AnimatorUpdateListener() {
                    public void onAnimationUpdate(ValueAnimator animator) {
                        anim2.setFraction(animator.getAnimatedFraction());
                        Impl21.dispatchOnProgress(view, Impl21.interpolateInsets(targetInsets, startingInsets, anim2.getInterpolatedFraction(), animationMask), Collections.singletonList(anim2));
                    }
                };
                WindowInsetsCompat targetInsets2 = targetInsets;
                WindowInsetsCompat windowInsetsCompat = startingInsets;
                int i = animationMask;
                ...
                OneShotPreDrawListener.add(v, new Runnable() {
                    public void run() {
                        Impl21.dispatchOnStart(view2, anim3, animationBounds2);
                        animator2.start();
                    }
                });
