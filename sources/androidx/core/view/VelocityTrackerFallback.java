...
class VelocityTrackerFallback {
    ...
    public void computeCurrentVelocity(int units, float maxVelocity) {
        this.mLastComputedVelocity = getCurrentVelocity() * ((float) units);
    ...
    private float getCurrentVelocity() {
        ...
        for (int i = 0; i < this.mDataPointsBufferSize - 1; i++) {
            ...
            if (this.mEventTimes[nextIndex] != eventTime) {
                ...
                float vCurr = this.mMovements[nextIndex] / ((float) (this.mEventTimes[nextIndex] - eventTime));
    ...
    private static float kineticEnergyToVelocity(float work) {
        return (work < 0.0f ? -1.0f : 1.0f) * ((float) Math.sqrt((double) (Math.abs(work) * 2.0f)));
    }
}
