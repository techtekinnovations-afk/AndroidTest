...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        velocityTrackerMotionEvent.offsetLocation(0.0f, (float) this.mNestedYOffset);
        switch (actionMasked) {
            case 0:
                if (getChildCount() != 0) {
            ...
            case 1:
                ...
                if (Math.abs(initialVelocity) >= this.mMinimumVelocity) {
                    if (!edgeEffectFling(initialVelocity) && !dispatchNestedPreFling(0.0f, (float) (-initialVelocity))) {
            ...
            case 2:
                ...
                if (activePointerIndex != -1) {
                    ...
                    if (this.mIsBeingDragged) {
                        int scrollOffset = scrollBy(deltaY2, (int) motionEvent.getX(activePointerIndex), 0, false);
    ...
    private int scrollBy(int verticalScrollDistance, int x, int touchType, boolean isSourceMouseOrKeyboard) {
        ...
        int i3 = i2;
        if (dispatchNestedPreScroll(0, verticalScrollDistance, this.mScrollConsumed, this.mScrollOffset, i2)) {
            int totalScrollOffset2 = 0 + this.mScrollOffset[1];
            verticalScrollDistance2 = verticalScrollDistance - this.mScrollConsumed[1];
        ...
        int verticalScrollDistance3 = verticalScrollDistance2;
        int initialScrollY2 = initialScrollY;
        int scrollRangeY2 = scrollRangeY;
        boolean hitScrollBarrier = overScrollByCompat(0, verticalScrollDistance2, 0, initialScrollY, 0, scrollRangeY, 0, 0, true) && !hasNestedScrollingParent(i3);
        int scrollYDelta = getScrollY() - initialScrollY2;
        this.mScrollConsumed[1] = 0;
        int i4 = i3;
        dispatchNestedScroll(0, scrollYDelta, 0, verticalScrollDistance3 - scrollYDelta, this.mScrollOffset, i4, this.mScrollConsumed);
        int i5 = i4;
        int totalScrollOffset3 = totalScrollOffset + this.mScrollOffset[1];
        int verticalScrollDistance4 = verticalScrollDistance3 - this.mScrollConsumed[1];
        int newScrollY = initialScrollY2 + verticalScrollDistance4;
        if (newScrollY < 0) {
            if (canOverscroll) {
                EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, ((float) (-verticalScrollDistance4)) / ((float) getHeight()), ((float) i) / ((float) getWidth()));
                if (!this.mEdgeGlowBottom.isFinished()) {
                    this.mEdgeGlowBottom.onRelease();
                }
            }
        } else if (newScrollY > scrollRangeY2 && canOverscroll) {
            EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, ((float) verticalScrollDistance4) / ((float) getHeight()), 1.0f - (((float) i) / ((float) getWidth())));
        ...
        if (hitScrollBarrier && i5 == 0 && this.mVelocityTracker != null) {
            this.mVelocityTracker.clear();
        }
        if (i5 == 1) {
            stopNestedScroll(i5);
    ...
    private float getSplineFlingDistance(int velocity) {
        return (float) (((double) (this.mPhysicalCoeff * SCROLL_FRICTION)) * Math.exp((((double) DECELERATION_RATE) / (((double) DECELERATION_RATE) - 1.0d)) * Math.log((double) ((((float) Math.abs(velocity)) * INFLEXION) / (this.mPhysicalCoeff * SCROLL_FRICTION)))));
    }
    ...
    public boolean onGenericMotionEvent(MotionEvent motionEvent) {
        ...
        scrollBy(-((int) (getVerticalScrollFactorCompat() * verticalScroll)), x, 1, MotionEventCompat.isFromSource(motionEvent, 8194));
        if (flingAxis != 0) {
    ...
    public boolean overScrollByCompat(int deltaX, int deltaY, int scrollX, int scrollY, int scrollRangeX, int scrollRangeY, int maxOverScrollX, int maxOverScrollY, boolean isTouchEvent) {
        ...
        int newScrollX3 = scrollX + deltaX;
        if (!overScrollHorizontal) {
            maxOverScrollX2 = 0;
        } else {
            maxOverScrollX2 = maxOverScrollX;
        }
        int newScrollY3 = scrollY + deltaY;
        ...
        if (newScrollX3 > right) {
            newScrollX = right;
            clampedX = true;
        } else if (newScrollX3 < left) {
            newScrollX = left;
            clampedX = true;
        } else {
            newScrollX = newScrollX3;
            clampedX = false;
        }
        if (newScrollY3 > bottom) {
            newScrollY = bottom;
            clampedY = true;
        } else if (newScrollY3 < top) {
            newScrollY = top;
            clampedY = true;
        } else {
            newScrollY = newScrollY3;
            clampedY = false;
        }
        if (!clampedY || hasNestedScrollingParent(1)) {
            newScrollX2 = newScrollX;
        ...
        onOverScrolled(newScrollX2, newScrollY2, clampedX, clampedY);
    ...
    private boolean scrollAndFocus(int direction, int top, int bottom) {
        ...
        if (top < containerTop || bottom > containerBottom) {
            scrollBy(up ? top - containerTop : bottom - containerBottom, 0, 1, true);
        } else {
            handled = false;
        }
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (nextFocused == null || !isWithinDeltaOfScreen(nextFocused, maxJump, getHeight())) {
            ...
            scrollBy(direction == 130 ? scrollDelta : -scrollDelta, 0, 1, true);
        } else {
    ...
    public void computeScroll() {
        ...
        if (!this.mScroller.isFinished()) {
            ...
            int scrolledByMe = unconsumed - this.mScrollConsumed[1];
            int range2 = getScrollRange();
            if (scrolledByMe != 0) {
                int oldScrollY = getScrollY();
                overScrollByCompat(0, scrolledByMe, getScrollX(), oldScrollY, 0, range2, 0, 0, false);
                range = range2;
                int scrolledByMe2 = getScrollY() - oldScrollY;
                int unconsumed2 = scrolledByMe - scrolledByMe2;
                this.mScrollConsumed[1] = 0;
                dispatchNestedScroll(0, scrolledByMe2, 0, unconsumed2, this.mScrollOffset, 1, this.mScrollConsumed);
                scrolledByMe = unconsumed2 - this.mScrollConsumed[1];
            } else {
                range = range2;
            }
            if (scrolledByMe != 0) {
    ...
    public void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (!this.mIsLaidOut) {
            ...
            int parentSpace = ((b2 - r2) - getPaddingTop()) - getPaddingBottom();
