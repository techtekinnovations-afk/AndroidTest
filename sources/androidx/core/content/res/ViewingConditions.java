...
final class ViewingConditions {
    ...
    static ViewingConditions make(float[] whitepoint, float adaptingLuminance, float backgroundLstar, float surround, boolean discountingIlluminant) {
        ...
        float ncb = nbb;
        float nbb2 = nbb;
        float[] rgbAFactors = {(float) Math.pow(((double) ((rgbD[0] * fl) * rW2)) / 100.0d, 0.42d), (float) Math.pow(((double) ((rgbD[1] * fl) * gW)) / 100.0d, 0.42d), (float) Math.pow(((double) ((rgbD[2] * fl) * bW)) / 100.0d, 0.42d)};
        ...
        float[][] fArr = matrix;
        float[] fArr2 = xyz;
        float[] fArr3 = rgbAFactors;
        return new ViewingConditions(n, ((rgbA[0] * 2.0f) + rgbA[1] + (rgbA[2] * 0.05f)) * nbb2, nbb2, ncb, c, f2, rgbD, fl, (float) Math.pow((double) fl, 0.25d), z);
    }
}
