...
public class TooltipDrawable extends MaterialShapeDrawable implements TextDrawableHelper.TextDrawableDelegate {
    ...
    private void loadFromAttributes(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        setFillColor(ColorStateList.valueOf(a.getColor(R.styleable.Tooltip_backgroundTint, MaterialColors.layer(ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, 16842801, cls.getCanonicalName()), 229), ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, R.attr.colorOnBackground, cls.getCanonicalName()), 153)))));
    ...
    public void draw(Canvas canvas) {
        canvas.save();
        canvas.scale(this.tooltipScaleX, this.tooltipScaleY, ((float) getBounds().left) + (((float) getBounds().width()) * this.tooltipPivotX), ((float) getBounds().top) + (((float) getBounds().height()) * this.tooltipPivotY));
    ...
    private EdgeTreatment createMarkerEdge() {
        float maxArrowOffset = (float) ((((double) getBounds().width()) - (((double) this.arrowSize) * Math.sqrt(2.0d))) / 2.0d);
    ...
    private void drawText(Canvas canvas) {
        if (this.text != null) {
            ...
            canvas.drawText(this.text, 0, this.text.length(), (float) bounds.centerX(), (float) y, this.textDrawableHelper.getTextPaint());
        }
    }
