...
public class ShapePath {
    ...
    public void lineTo(float x1, float y1, float x2, float y2) {
        if ((Math.abs(x1 - getEndX()) >= 0.001f || Math.abs(y1 - getEndY()) >= 0.001f) && (Math.abs(x1 - x2) >= 0.001f || Math.abs(y1 - y2) >= 0.001f)) {
            ...
            float unused = operation1.x = x1;
            float unused2 = operation1.y = y1;
            this.operations.add(operation1);
            PathLineOperation operation2 = new PathLineOperation();
            float unused3 = operation2.x = x2;
            float unused4 = operation2.y = y2;
    ...
    public void cubicToPoint(float controlX1, float controlY1, float controlX2, float controlY2, float toX, float toY) {
        float toX2 = toX;
        float toY2 = toY;
        this.operations.add(new PathCubicOperation(controlX1, controlY1, controlX2, controlY2, toX2, toY2));
        this.containsIncompatibleShadowOp = true;
        setEndX(toX2);
        setEndY(toY2);
    }
    ...
    static class InnerCornerShadowOperation extends ShadowCompatOperation {
        ...
        public void draw(Matrix transform, ShadowRenderer shadowRenderer, int shadowElevation, Canvas canvas) {
            ...
            int i2 = shadowElevation;
            Canvas canvas2 = canvas;
            float sweepAngle = getSweepAngle();
            if (sweepAngle <= 0.0f) {
                double length1 = Math.hypot((double) (this.operation1.x - this.startX), (double) (this.operation1.y - this.startY));
                double length2 = Math.hypot((double) (this.operation2.x - this.operation1.x), (double) (this.operation2.y - this.operation1.y));
                float arcRadius = (float) Math.min((double) i2, Math.min(length1, length2));
                double retractLength = ((double) arcRadius) * Math.tan(Math.toRadians((double) ((-sweepAngle) / 2.0f)));
                ...
                float[] fArr = {(float) (((double) arcRadius) + retractLength2), arcRadius * 2.0f};
                float f = sweepAngle2;
                float[] fArr2 = fArr;
                float sweepAngle3 = f;
                RectF rect2 = rect;
                ...
                float f2 = arcRadius;
                double retractLength3 = retractLength2;
                double retractLength4 = length1;
                shadowRenderer3.drawInnerCornerShadow(canvas2, matrix2, rect2, (int) arcRadius, 450.0f, sweepAngle3, fArr2);
                if (length2 > retractLength3) {
                    RectF rect22 = new RectF(0.0f, 0.0f, (float) (length2 - retractLength3), 0.0f);
                    this.renderMatrix.set(matrix);
                    this.renderMatrix.preTranslate(this.operation1.x, this.operation1.y);
                    this.renderMatrix.preRotate(getEndAngle());
                    this.renderMatrix.preTranslate((float) retractLength3, 0.0f);
                    shadowRenderer3.drawEdgeShadow(canvas2, this.renderMatrix, rect22, i);
                }
            }
        }
    ...
    static class ArcShadowOperation extends ShadowCompatOperation {
        ...
        public void draw(Matrix transform, ShadowRenderer shadowRenderer, int shadowElevation, Canvas canvas) {
            ...
            ShadowRenderer shadowRenderer2 = shadowRenderer;
            Canvas canvas2 = canvas;
            shadowRenderer2.drawCornerShadow(canvas2, transform, new RectF(this.operation.getLeft(), this.operation.getTop(), this.operation.getRight(), this.operation.getBottom()), shadowElevation, startAngle, sweepAngle);
        }
    }
