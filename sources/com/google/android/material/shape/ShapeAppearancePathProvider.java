...
public class ShapeAppearancePathProvider {
    ...
    public void calculatePath(ShapeAppearanceModel shapeAppearanceModel, float interpolation, RectF bounds, PathListener pathListener, Path path) {
        calculatePath(shapeAppearanceModel, (float[]) null, interpolation, bounds, pathListener, path);
    }

    public void calculatePath(ShapeAppearanceModel shapeAppearanceModel, float[] cornerSizeOverrides, float interpolation, RectF bounds, PathListener pathListener, Path path) {
        path.rewind();
        this.overlappedEdgePath.rewind();
        this.boundsPath.rewind();
        this.boundsPath.addRect(bounds, Path.Direction.CW);
        Path path2 = path;
        ShapeAppearancePathSpec spec = new ShapeAppearancePathSpec(shapeAppearanceModel, interpolation, bounds, pathListener, path2);
    ...
    private void setCornerPathAndTransform(ShapeAppearancePathSpec spec, int index, float[] cornerSizes) {
        CornerSize cornerSize;
        if (cornerSizes == null) {
            cornerSize = getCornerSizeForIndex(index, spec.shapeAppearanceModel);
        } else {
            cornerSize = new ClampedCornerSize(cornerSizes[index]);
        }
        getCornerTreatmentForIndex(index, spec.shapeAppearanceModel).getCornerPath(this.cornerPaths[index], 90.0f, spec.interpolation, spec.bounds, cornerSize);
    ...
    private void appendEdgePath(ShapeAppearancePathSpec spec, int index) {
        ...
        float edgeLength = Math.max(((float) Math.hypot((double) (this.scratch[0] - this.scratch2[0]), (double) (this.scratch[1] - this.scratch2[1]))) - 0.001f, 0.0f);
    ...
    public CornerSize getCornerSizeForIndex(int index, ShapeAppearanceModel shapeAppearanceModel) {
        switch (index) {
            case 1:
