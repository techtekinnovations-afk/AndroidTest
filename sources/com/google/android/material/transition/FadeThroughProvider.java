...
public final class FadeThroughProvider implements VisibilityAnimatorProvider {
    ...
    private static Animator createFadeThroughAnimator(View view, float startValue, float endValue, float startFraction, float endFraction, final float originalAlpha) {
        ValueAnimator animator = ValueAnimator.ofFloat(new float[]{0.0f, 1.0f});
        final View view2 = view;
        final float startValue2 = startValue;
        final float endValue2 = endValue;
        final float startFraction2 = startFraction;
        final float endFraction2 = endFraction;
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            public void onAnimationUpdate(ValueAnimator animation) {
                view2.setAlpha(TransitionUtils.lerp(startValue2, endValue2, startFraction2, endFraction2, ((Float) animation.getAnimatedValue()).floatValue()));
            }
        });
        animator.addListener(new AnimatorListenerAdapter() {
            public void onAnimationEnd(Animator animation) {
                view2.setAlpha(originalAlpha);
            }
        });
        return animator;
    }
