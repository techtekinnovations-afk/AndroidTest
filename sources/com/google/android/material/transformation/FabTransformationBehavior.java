...
public abstract class FabTransformationBehavior extends ExpandableTransformationBehavior {
    ...
    public AnimatorSet onCreateExpandedStateChangeAnimation(View dependency, View child, boolean expanded, boolean isAnimating) {
        ...
        List<Animator.AnimatorListener> arrayList = new ArrayList<>();
        final View dependency2 = dependency;
        final View child2 = child;
        final boolean expanded2 = expanded;
        boolean isAnimating2 = isAnimating;
        createElevationAnimation(dependency2, child2, expanded2, isAnimating2, spec, animations, arrayList);
        RectF childBounds = this.tmpRectF1;
        createTranslationAnimation(dependency2, child2, expanded2, isAnimating2, spec, animations, arrayList, childBounds);
        boolean isAnimating3 = isAnimating2;
        RectF childBounds2 = childBounds;
        float childWidth = childBounds2.width();
        float childHeight = childBounds2.height();
        FabTransformationSpec spec2 = spec;
        createDependencyTranslationAnimation(dependency2, child2, expanded2, spec2, animations);
        FabTransformationSpec spec3 = spec2;
        boolean isAnimating4 = isAnimating3;
        createIconFadeAnimation(dependency2, child2, expanded2, isAnimating4, spec3, animations, arrayList);
        List<Animator> list = animations;
        List<Animator.AnimatorListener> listeners = arrayList;
        float childWidth2 = childWidth;
        float childHeight2 = childHeight;
        createExpansionAnimation(dependency2, child2, expanded2, isAnimating4, spec3, childWidth2, childHeight2, list, listeners);
        float f = childWidth2;
        float f2 = childHeight2;
        List<Animator> animations2 = list;
        List<Animator.AnimatorListener> listeners2 = listeners;
        createColorAnimation(dependency2, child2, expanded2, isAnimating4, spec3, animations2, listeners2);
        createChildrenFadeAnimation(dependency2, child2, expanded2, isAnimating4, spec3, animations2, listeners2);
        AnimatorSet set = new AnimatorSet();
        AnimatorSetCompat.playTogether(set, animations2);
        set.addListener(new AnimatorListenerAdapter() {
            public void onAnimationStart(Animator animation) {
                if (expanded2) {
                    child2.setVisibility(0);
                    dependency2.setAlpha(0.0f);
                    dependency2.setVisibility(4);
                }
            }
            ...
            public void onAnimationEnd(Animator animation) {
                if (!expanded2) {
                    ...
                    dependency2.setAlpha(1.0f);
                    dependency2.setVisibility(0);
                }
            }
        });
        int count = listeners2.size();
        for (int i = 0; i < count; i++) {
            set.addListener(listeners2.get(i));
        }
        return set;
    }
    ...
    private void createDependencyTranslationAnimation(View dependency, View child, boolean expanded, FabTransformationSpec spec, List<Animator> animations) {
        ...
        ValueAnimator translationXAnimator = ObjectAnimator.ofFloat(dependency, View.TRANSLATION_X, new float[]{expanded ? translationX : this.dependencyOriginalTranslationX});
        ValueAnimator translationYAnimator = ObjectAnimator.ofFloat(dependency, View.TRANSLATION_Y, new float[]{expanded ? translationY : this.dependencyOriginalTranslationY});
    ...
    private void createTranslationAnimation(View dependency, View child, boolean expanded, boolean currentlyAnimating, FabTransformationSpec spec, List<Animator> animations, List<Animator.AnimatorListener> list, RectF childBounds) {
        ...
        if (z) {
            ...
            calculateChildVisibleBoundsAtEndOfExpansion(view2, fabTransformationSpec, translationXTiming, translationYTiming, -translationX, -translationY, 0.0f, 0.0f, childBounds);
            ...
            ValueAnimator ofFloat = ObjectAnimator.ofFloat(view2, View.TRANSLATION_Y, new float[]{-translationY});
    ...
    private void createExpansionAnimation(View dependency, View child, boolean expanded, boolean currentlyAnimating, FabTransformationSpec spec, float childWidth, float childHeight, List<Animator> animations, List<Animator.AnimatorListener> listeners) {
        ...
        if (view2 instanceof CircularRevealWidget) {
            final CircularRevealWidget circularRevealChild = (CircularRevealWidget) view2;
            ...
            if (expanded) {
                ...
                float revealCenterX2 = revealCenterX;
                float revealCenterY2 = revealCenterY;
                Animator animator2 = CircularRevealCompat.createCircularReveal(circularRevealChild, revealCenterX2, revealCenterY2, MathUtils.distanceToFurthestCorner(revealCenterX, revealCenterY, 0.0f, 0.0f, childWidth, childHeight));
                ...
                createPreFillRadialExpansion(view2, timing2.getDelay(), (int) revealCenterX2, (int) revealCenterY2, fromRadius, animations);
                float f = revealCenterX2;
                timing = timing2;
                float f2 = revealCenterY2;
                animator = animator2;
            } else {
                float revealCenterX3 = revealCenterX;
                float revealCenterY3 = revealCenterY;
                float fromRadius2 = circularRevealChild.getRevealInfo().radius;
                float toRadius = dependencyRadius;
                Animator animator3 = CircularRevealCompat.createCircularReveal(circularRevealChild, revealCenterX3, revealCenterY3, toRadius);
                createPreFillRadialExpansion(child, timing2.getDelay(), (int) revealCenterX3, (int) revealCenterY3, fromRadius2, animations);
                float f3 = fromRadius2;
                float revealCenterX4 = revealCenterX3;
                float f4 = revealCenterX4;
                timing = timing2;
                float f5 = revealCenterY3;
                createPostFillRadialExpansion(child, timing2.getDelay(), timing2.getDuration(), fabTransformationSpec.timings.getTotalDuration(), (int) revealCenterX4, (int) revealCenterY3, toRadius, animations);
                animator = animator3;
            }
    ...
    private void createChildrenFadeAnimation(View unusedDependency, View child, boolean expanded, boolean currentlyAnimating, FabTransformationSpec spec, List<Animator> animations, List<Animator.AnimatorListener> list) {
        ...
        if (child instanceof ViewGroup) {
            boolean z = child instanceof CircularRevealWidget;
