...
public class ElevationOverlayProvider {
    ...
    public int compositeOverlay(int backgroundColor, float elevation) {
        ...
        if (overlayAlphaFraction > 0.0f && this.elevationOverlayAccentColor != 0) {
            overlayColorOpaque = MaterialColors.layer(overlayColorOpaque, ColorUtils.setAlphaComponent(this.elevationOverlayAccentColor, OVERLAY_ACCENT_COLOR_ALPHA));
        }
    ...
    public float calculateOverlayAlphaFraction(float elevation) {
        ...
        return Math.min(((((float) Math.log1p((double) (elevation / this.displayDensity))) * FORMULA_MULTIPLIER) + FORMULA_OFFSET) / 100.0f, 1.0f);
    }
