...
public abstract class BaseProgressIndicator<S extends BaseProgressIndicatorSpec> extends ProgressBar {
    ...
    protected BaseProgressIndicator(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.minHideDelay = Math.min(a.getInt(R.styleable.BaseProgressIndicator_minHideDelay, -1), 1000);
        a.recycle();
    ...
    public void setTrackCornerRadiusFraction(float fraction) {
        if (this.spec.trackCornerRadiusFraction != fraction) {
            this.spec.trackCornerRadiusFraction = Math.min(fraction, 0.5f);
    ...
    public void setWaveAmplitude(int waveAmplitude) {
        if (this.spec.waveAmplitude != waveAmplitude) {
            this.spec.waveAmplitude = Math.abs(waveAmplitude);
            requestLayout();
        }
    }
    ...
    public void setWavelengthDeterminate(int wavelength) {
        if (this.spec.wavelengthDeterminate != wavelength) {
            this.spec.wavelengthDeterminate = Math.abs(wavelength);
            if (!isIndeterminate()) {
                requestLayout();
            }
        }
    ...
    public void setWavelengthIndeterminate(int wavelength) {
        if (this.spec.wavelengthIndeterminate != wavelength) {
            this.spec.wavelengthIndeterminate = Math.abs(wavelength);
            if (isIndeterminate()) {
                requestLayout();
            }
        }
    ...
    public void setIndeterminateAnimatorDurationScale(float indeterminateAnimatorDurationScale) {
        if (this.spec.indeterminateAnimatorDurationScale != indeterminateAnimatorDurationScale) {
            this.spec.indeterminateAnimatorDurationScale = indeterminateAnimatorDurationScale;
            getIndeterminateDrawable().getAnimatorDelegate().invalidateSpecValues();
        }
    }
