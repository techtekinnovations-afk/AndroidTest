...
public class HctSolver {
    ...
    static double chromaticAdaptation(double component) {
        ...
        return ((((double) MathUtils.signum(component)) * 400.0d) * af) / (27.13d + af);
    }
    ...
    static double hueOf(double[] linrgb) {
        ...
        return Math.atan2(((rA + gA) - (2.0d * bA)) / 9.0d, (((rA * 11.0d) + (-12.0d * gA)) + bA) / 11.0d);
    }
    ...
    static double[] nthVertex(double y, int n) {
        ...
        double coordB = 0.0d;
        double coordA = i % 4 <= 1 ? 0.0d : 100.0d;
        if (i % 2 != 0) {
            coordB = 100.0d;
        }
        if (i < 4) {
            ...
            double r3 = coordA;
            double g3 = coordB;
            double b3 = ((y - (r3 * kR)) - (g3 * kG)) / kB;
            if (!isBounded(b3)) {
                return new double[]{-1.0d, -1.0d, -1.0d};
            }
            return new double[]{r3, g3, b3};
        }
    }

    /* JADX WARNING: Code restructure failed: missing block: B:11:0x003a, code lost:
        if (areInCyclicOrder(r2, r4, r6) != false) goto L_0x0040;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    static double[][] bisectToSegment(double r17, double r19) {
        /*
            r0 = 3
            double[] r0 = new double[r0]
            r0 = {-4616189618054758400, -4616189618054758400, -4616189618054758400} // fill-array
            r1 = r0
            r2 = 0
            r4 = 0
            r6 = 0
        ...
        L_0x0033:
            ...
            boolean r14 = areInCyclicOrder(r2, r4, r6)
            r15 = r6
    ...
    static double[] midpoint(double[] a, double[] b) {
        return new double[]{(a[0] + b[0]) / 2.0d, (a[1] + b[1]) / 2.0d, (a[2] + b[2]) / 2.0d};
    }
    ...
    static double[] bisectToLimit(double y, double targetHue) {
        ...
        for (int axis = 0; axis < 3; axis++) {
            if (left[axis] != right[axis]) {
                ...
                int rPlane2 = rPlane;
                double leftHue2 = d;
                for (int i = 0; i < 8 && Math.abs(rPlane2 - lPlane2) > 1; i++) {
                    int mPlane = (int) Math.floor(((double) (lPlane2 + rPlane2)) / 2.0d);
                    ...
                    double d2 = midPlaneCoordinate;
                    if (areInCyclicOrder(leftHue2, targetHue, midHue)) {
                        right = mid;
                        rPlane2 = mPlane;
                    } else {
                        left = mid;
    ...
    static double inverseChromaticAdaptation(double adapted) {
        ...
        return ((double) MathUtils.signum(adapted)) * Math.pow(Math.max(0.0d, (27.13d * adaptedAbs) / (400.0d - adaptedAbs)), 2.380952380952381d);
    }
    ...
    static int findResultByJ(double hueRadians, double chroma, double y) {
        ...
        double d2 = 2.0d;
        double p1 = 3846.153846153846d * (Math.cos(hueRadians + 2.0d) + 3.8d) * 0.25d * viewingConditions.getNc() * viewingConditions.getNcb();
        ...
        while (iterationRound < 5) {
            ...
            double d4 = d2;
            double j2 = j;
            double t = Math.pow(((chroma == 0.0d || j == 0.0d) ? 0.0d : chroma / Math.sqrt(jNormalized)) * tInnerCoeff, 1.1111111111111112d);
            double p2 = (viewingConditions.getAw() * Math.pow(jNormalized, (d3 / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double gamma = (((p2 + 0.305d) * 23.0d) * t) / (((23.0d * p1) + ((t * 11.0d) * hCos)) + ((108.0d * t) * hSin));
            double a = gamma * hCos;
            double b = gamma * hSin;
            double d5 = p2;
            double[] linrgb = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation((((p2 * 460.0d) + (451.0d * a)) + (288.0d * b)) / 1403.0d), inverseChromaticAdaptation((((p2 * 460.0d) - (891.0d * a)) - (261.0d * b)) / 1403.0d), inverseChromaticAdaptation((((460.0d * p2) - (220.0d * a)) - (6300.0d * b)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            if (linrgb[0] < 0.0d || linrgb[1] < 0.0d || linrgb[2] < 0.0d) {
                return 0;
            }
            double fnj = (linrgb[0] * Y_FROM_LINRGB[0]) + (linrgb[1] * Y_FROM_LINRGB[1]) + (linrgb[2] * Y_FROM_LINRGB[2]);
            if (fnj <= 0.0d) {
                return 0;
            }
            if (iterationRound != 4 && Math.abs(fnj - y) >= 0.002d) {
                j = j2 - (((fnj - y) * j2) / (fnj * d4));
                iterationRound++;
                d = d3;
                d2 = d4;
            } else if (linrgb[0] > 100.01d || linrgb[1] > 100.01d || linrgb[2] > 100.01d) {
                return 0;
            } else {
    ...
    public static int solveToInt(double hueDegrees, double chroma, double lstar) {
        ...
        } else if (lstar > 99.9999d) {
            double d = chroma;
        } else {
            double hueRadians = (MathUtils.sanitizeDegreesDouble(hueDegrees) / 180.0d) * 3.141592653589793d;
