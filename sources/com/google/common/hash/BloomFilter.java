...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
    ...
    public static <T> BloomFilter<T> readFrom(InputStream in, Funnel<? super T> funnel2) throws IOException {
        ...
        try {
            ...
            BloomFilterStrategies.LockFreeBitArray dataArray = new BloomFilterStrategies.LockFreeBitArray(LongMath.checkedMultiply((long) dataLength, 64));
            for (int i = 0; i < dataLength; i++) {
                dataArray.putData(i, din.readLong());
            }
            return new BloomFilter<>(dataArray, numHashFunctions2, funnel2, strategy2);
