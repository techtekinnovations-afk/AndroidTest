...
public final class Quantiles {
    ...
    public static final class ScaleAndIndexes {
        ...
        private ScaleAndIndexes(int scale2, int[] indexes2) {
            ...
            Preconditions.checkArgument(indexes2.length > 0 ? true : z, "Indexes must be a non empty array");
            this.scale = scale2;
        ...
        public Map<Integer, Double> computeInPlace(double... dataset) {
            ...
            ScaleAndIndexes scaleAndIndexes = this;
            double[] dArr = dataset;
            Preconditions.checkArgument(dArr.length > 0, "Cannot calculate quantiles of an empty dataset");
            if (Quantiles.containsNaN(dArr)) {
                Map<Integer, Double> nanMap = new LinkedHashMap<>();
                for (int index : scaleAndIndexes.indexes) {
                    nanMap.put(Integer.valueOf(index), Double.valueOf(Double.NaN));
                }
                return Collections.unmodifiableMap(nanMap);
            }
            int[] quotients = new int[scaleAndIndexes.indexes.length];
            int[] remainders = new int[scaleAndIndexes.indexes.length];
            int[] requiredSelections2 = new int[(scaleAndIndexes.indexes.length * 2)];
            int requiredSelectionsCount = 0;
            for (int i = 0; i < scaleAndIndexes.indexes.length; i++) {
                long numerator = ((long) scaleAndIndexes.indexes[i]) * ((long) (dArr.length - 1));
                int quotient = (int) LongMath.divide(numerator, (long) scaleAndIndexes.scale, RoundingMode.DOWN);
                int remainder = (int) (numerator - (((long) quotient) * ((long) scaleAndIndexes.scale)));
                quotients[i] = quotient;
                remainders[i] = remainder;
                requiredSelections2[requiredSelectionsCount] = quotient;
                requiredSelectionsCount++;
                if (remainder != 0) {
                    requiredSelections2[requiredSelectionsCount] = quotient + 1;
                    requiredSelectionsCount++;
                }
            }
            Arrays.sort(requiredSelections2, 0, requiredSelectionsCount);
            Quantiles.selectAllInPlace(requiredSelections2, 0, requiredSelectionsCount - 1, dArr, 0, dArr.length - 1);
            Map<Integer, Double> ret = new LinkedHashMap<>();
            int i2 = 0;
            while (i2 < scaleAndIndexes.indexes.length) {
                int quotient2 = quotients[i2];
                int remainder2 = remainders[i2];
                if (remainder2 == 0) {
                    ret.put(Integer.valueOf(scaleAndIndexes.indexes[i2]), Double.valueOf(dataset[quotient2]));
                    requiredSelections = requiredSelections2;
                } else {
                    requiredSelections = requiredSelections2;
                    ret.put(Integer.valueOf(scaleAndIndexes.indexes[i2]), Double.valueOf(Quantiles.interpolate(dataset[quotient2], dataset[quotient2 + 1], (double) remainder2, (double) scaleAndIndexes.scale)));
                }
                i2++;
                scaleAndIndexes = this;
                requiredSelections2 = requiredSelections;
            }
