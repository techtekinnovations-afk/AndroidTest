...
public final class StatsAccumulator {
    ...
    private void merge(long otherCount, double otherMean, double otherSumOfSquaresOfDeltas, double otherMin, double otherMax) {
        ...
        if (!Doubles.isFinite(this.mean) || !Doubles.isFinite(d)) {
            ...
            double delta = d - this.mean;
            this.mean += (((double) j) * delta) / ((double) this.count);
            this.sumOfSquaresOfDeltas = otherSumOfSquaresOfDeltas + ((d - this.mean) * delta * ((double) j)) + this.sumOfSquaresOfDeltas;
        }
