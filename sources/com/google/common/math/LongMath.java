...
public final class LongMath {
    ...
    public static long divide(long p, long q, RoundingMode mode) {
        ...
        switch (AnonymousClass1.$SwitchMap$java$math$RoundingMode[roundingMode.ordinal()]) {
            ...
            case 8:
                ...
                } else if (roundingMode != RoundingMode.HALF_UP && (roundingMode != RoundingMode.HALF_EVEN || (1 & div) == 0)) {
                    increment = false;
                    break;
                }
                break;
    ...
    public static long checkedMultiply(long a, long b) {
        ...
        long b3 = b2;
        long a3 = a2;
        MathPreconditions.checkNoOverflow((a3 >= 0) | (b3 != Long.MIN_VALUE), "checkedMultiply", a3, b3);
        long result = a3 * b3;
        MathPreconditions.checkNoOverflow(a3 == 0 || result / a3 == b3, "checkedMultiply", a3, b3);
        return result;
    }
            ...
            while (true) {
                switch (k2) {
                    ...
                    default:
                        ...
                        int k3 = k2 >> 1;
                        if (k3 > 0) {
                            MathPreconditions.checkNoOverflow(-3037000499L <= b2 && b2 <= FLOOR_SQRT_MAX_LONG, "checkedPow", b2, (long) k3);
                            b2 *= b2;
                            k2 = k3;
                        } else {
                            k2 = k3;
        ...
        switch (k) {
            ...
            default:
                ...
                for (int i2 = 2; i2 <= k; i2++) {
                    result2 = (result2 * ((long) n4)) / ((long) i2);
                    n4--;
                }
                return result2;
        }
    ...
    public static double roundToDouble(long x, RoundingMode mode) {
        ...
        long deltaToCeiling;
        long j2 = x;
        double roundArbitrarily = (double) j2;
        long roundArbitrarilyAsLong = (long) roundArbitrarily;
        if (roundArbitrarilyAsLong == Long.MAX_VALUE) {
            cmpXToRoundArbitrarily = -1;
        } else {
            cmpXToRoundArbitrarily = Longs.compare(j2, roundArbitrarilyAsLong);
        }
        switch (AnonymousClass1.$SwitchMap$java$math$RoundingMode[mode.ordinal()]) {
            case 1:
                double roundArbitrarily2 = roundArbitrarily;
                MathPreconditions.checkRoundingUnnecessary(cmpXToRoundArbitrarily == 0);
                return roundArbitrarily2;
            case 2:
                double roundArbitrarily3 = roundArbitrarily;
                if (x < 0) {
                    return cmpXToRoundArbitrarily <= 0 ? roundArbitrarily3 : Math.nextUp(roundArbitrarily3);
                }
                if (cmpXToRoundArbitrarily >= 0) {
                    return roundArbitrarily3;
                }
                return DoubleUtils.nextDown(roundArbitrarily3);
            case 3:
                double roundArbitrarily4 = roundArbitrarily;
                if (cmpXToRoundArbitrarily >= 0) {
                    return roundArbitrarily4;
                }
                return DoubleUtils.nextDown(roundArbitrarily4);
            case 4:
                double roundArbitrarily5 = roundArbitrarily;
                if (x >= 0) {
                    return cmpXToRoundArbitrarily <= 0 ? roundArbitrarily5 : Math.nextUp(roundArbitrarily5);
                }
                if (cmpXToRoundArbitrarily >= 0) {
                    return roundArbitrarily5;
                }
                return DoubleUtils.nextDown(roundArbitrarily5);
            case 5:
                double roundArbitrarily6 = roundArbitrarily;
                return cmpXToRoundArbitrarily <= 0 ? roundArbitrarily6 : Math.nextUp(roundArbitrarily6);
            case 6:
            case 7:
            case 8:
                if (cmpXToRoundArbitrarily >= 0) {
                ...
                long deltaToCeiling2 = roundCeiling - x;
                if (roundCeiling == j) {
                    double d = roundArbitrarily;
                    deltaToCeiling = deltaToCeiling2 + 1;
                } else {
                    double d2 = roundArbitrarily;
                    deltaToCeiling = deltaToCeiling2;
                }
