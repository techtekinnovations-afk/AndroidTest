...
final class DirectedGraphConnections<N, V> implements GraphConnections<N, V> {
    ...
    private DirectedGraphConnections(Map<N, Object> adjacentNodeValues2, @CheckForNull List<NodeConnection<N>> orderedNodeConnections2, int predecessorCount2, int successorCount2) {
    ...
    public Iterator<EndpointPair<N>> incidentEdgeIterator(N thisNode) {
        ...
        Preconditions.checkNotNull(thisNode);
        if (this.orderedNodeConnections == null) {
            resultWithDoubleSelfLoop = Iterators.concat(Iterators.transform(predecessors().iterator(), new DirectedGraphConnections$$ExternalSyntheticLambda0(thisNode)), Iterators.transform(successors().iterator(), new DirectedGraphConnections$$ExternalSyntheticLambda1(thisNode)));
        } else {
            resultWithDoubleSelfLoop = Iterators.transform(this.orderedNodeConnections.iterator(), new DirectedGraphConnections$$ExternalSyntheticLambda2(thisNode));
        }
