...
final class SchemaUtil {
    ...
    static int computeSizeStringList(int fieldNumber, List<?> list) {
        ...
        if (list instanceof LazyStringList) {
            ...
            for (int i3 = 0; i3 < length; i3++) {
                Object value = lazyList.getRaw(i3);
                if (value instanceof ByteString) {
                    i2 = CodedOutputStream.computeBytesSizeNoTag((ByteString) value);
                } else {
                    i2 = CodedOutputStream.computeStringSizeNoTag((String) value);
                }
                size += i2;
            }
        } else {
            for (int i4 = 0; i4 < length; i4++) {
                ...
                if (obj instanceof ByteString) {
                    ...
                    i = CodedOutputStream.computeStringSizeNoTag((String) obj);
                }
                size += i;
            }
        }
    ...
    static int computeSizeMessageList(int fieldNumber, List<?> list) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            ...
            if (obj instanceof LazyFieldLite) {
                i = CodedOutputStream.computeLazyFieldSizeNoTag((LazyFieldLite) obj);
            } else {
                i = CodedOutputStream.computeMessageSizeNoTag((MessageLite) obj);
            }
            size += i;
        }
        return size;
    ...
    static int computeSizeMessageList(int fieldNumber, List<?> list, Schema schema) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            ...
            if (obj instanceof LazyFieldLite) {
                ...
                i = CodedOutputStream.computeMessageSizeNoTag((MessageLite) obj, schema);
            }
            size += i;
        }
        return size;
    ...
    static <T> void mergeMap(MapFieldSchema mapFieldSchema, T message, T o, long offset) {
        UnsafeUtil.putObject((Object) message, offset, mapFieldSchema.mergeFrom(UnsafeUtil.getObject((Object) message, offset), UnsafeUtil.getObject((Object) o, offset)));
    }
        ...
        if (enumList instanceof RandomAccess) {
            ...
            while (it.hasNext()) {
                ...
                if (enumMap.findValueByNumber(enumValue2) == null) {
                    unknownFields = storeUnknownEnum(containerMessage, number, enumValue2, unknownFields, unknownFieldSchema);
                    it.remove();
                }
            }
        }
