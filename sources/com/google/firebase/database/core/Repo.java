...
public class Repo implements PersistentConnection.Delegate {
    ...
    public void setValue(Path path, Node newValueUnresolved, DatabaseReference.CompletionListener onComplete) {
        ...
        Node newValue = ServerValues.resolveDeferredValueSnapshot(node, this.serverSyncTree.calcCompleteEventCache(path2, new ArrayList()), ServerValues.generateServerValues(this.serverClock));
        long writeId = getNextWriteId();
        final long writeId2 = writeId;
        Node node2 = newValue;
        postEvents(this.serverSyncTree.applyUserOverwrite(path, node, newValue, writeId, true, true));
        ...
        this.connection.put(path.asList(), node.getValue(true), new RequestResultCallback() {
            public void onRequestResult(String optErrorCode, String optErrorMessage) {
                ...
                Repo.this.ackWriteAndRerunTransactions(writeId2, path3, error);
                Repo.this.callOnComplete(completionListener, error, path3);
            }
        });
        rerunTransactions(abortTransactions(path3, -9));
    }
    ...
    public Task<DataSnapshot> getValue(final Query query) {
        ...
        scheduleNow(new Runnable() {
            public void run() {
                ...
                DataSnapshot persisted = Repo.this.serverSyncTree.persistenceServerCache(query);
                if (persisted.exists()) {
                    Repo.this.scheduleDelayed(new Repo$9$$ExternalSyntheticLambda0(source, persisted), 3000);
                }
                Repo.this.connection.get(query.getPath().asList(), query.getSpec().getParams().getWireProtocolParams()).addOnCompleteListener((Executor) ((DefaultRunLoop) Repo.this.ctx.getRunLoop()).getExecutorService(), new Repo$9$$ExternalSyntheticLambda1(this, source, persisted, query, this));
            }

            /* access modifiers changed from: package-private */
            /* renamed from: lambda$run$1$com-google-firebase-database-core-Repo$9  reason: not valid java name */
            public /* synthetic */ void m1785lambda$run$1$comgooglefirebasedatabasecoreRepo$9(TaskCompletionSource source, DataSnapshot persisted, Query query, Repo repo, Task task) {
                List<? extends Event> events;
    ...
    public void updateChildren(Path path, CompoundWrite updates, DatabaseReference.CompletionListener onComplete, Map<String, Object> unParsedUpdates) {
        ...
        if (updates.isEmpty()) {
            ...
            callOnComplete(onComplete, (DatabaseError) null, path);
            return;
        }
        CompoundWrite resolved = ServerValues.resolveDeferredValueMerge(updates, this.serverSyncTree, path, ServerValues.generateServerValues(this.serverClock));
        long writeId = getNextWriteId();
        List<? extends Event> events = this.serverSyncTree.applyUserMerge(path, updates, resolved, writeId, true);
        final long writeId2 = writeId;
        CompoundWrite compoundWrite = resolved;
        postEvents(events);
        ...
        this.connection.merge(path.asList(), map, new RequestResultCallback() {
            public void onRequestResult(String optErrorCode, String optErrorMessage) {
                ...
                Repo.this.ackWriteAndRerunTransactions(writeId2, path2, error);
    ...
    public void startTransaction(Path path, Transaction.Handler handler, boolean applyLocally) {
        ...
        AnonymousClass15 r5 = new ValueEventListener() {
        ...
        addEventCallback(new ValueEventRegistration(this, r5, watchRef.getSpec()));
        Transaction.Handler handler2 = handler;
        final Transaction.Handler handler3 = handler2;
        AnonymousClass15 r12 = r5;
        TransactionData transaction = new TransactionData(path2, handler2, r5, TransactionStatus.INITIALIZING, applyLocally, nextTransactionOrder());
        Node currentState = getLatestState(path);
        Node unused = transaction.currentInputSnapshot = currentState;
        try {
            result = handler3.doTransaction(InternalHelpers.createMutableData(currentState));
            if (result != null) {
                error = null;
                if (!result.isSuccess()) {
                    ...
                    postEvent(new Runnable() {
                        public void run() {
                            handler3.onComplete(innerClassError, false, snap);
                        }
                    });
                    return;
                }
    ...
    public void abortTransactionsAtNode(Tree<List<TransactionData>> node, int reason) {
        ...
        int i = reason;
        List<TransactionData> queue = tree.getValue();
        List<Event> events = new ArrayList<>();
        if (queue != null) {
            List<Runnable> callbacks = new ArrayList<>();
            if (i == -9) {
