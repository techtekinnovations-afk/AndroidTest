...
public class SyncTree {
    ...
    public List<? extends Event> applyUserOverwrite(Path path, Node newDataUnresolved, Node newData, long writeId, boolean visible, boolean persist) {
        ...
        return (List) this.persistenceManager.runInTransaction(new Callable<List<? extends Event>>() {
            public List<? extends Event> call() {
                ...
                SyncTree.this.pendingWriteTree.addOverwrite(path2, node2, Long.valueOf(j), z);
                if (!z) {
    ...
    public List<? extends Event> applyUserMerge(Path path, CompoundWrite unresolvedChildren, CompoundWrite children, long writeId, boolean persist) {
        final Path path2 = path;
        final CompoundWrite unresolvedChildren2 = unresolvedChildren;
        final CompoundWrite children2 = children;
        final long writeId2 = writeId;
        final boolean persist2 = persist;
        return (List) this.persistenceManager.runInTransaction(new Callable<List<? extends Event>>() {
            public List<? extends Event> call() throws Exception {
                if (persist2) {
                    SyncTree.this.persistenceManager.saveUserMerge(path2, unresolvedChildren2, writeId2);
                }
                SyncTree.this.pendingWriteTree.addMerge(path2, children2, Long.valueOf(writeId2));
                return SyncTree.this.applyOperationToSyncPoints(new Merge(OperationSource.USER, path2, children2));
            }
        });
    }
    ...
    public List<? extends Event> ackUserWrite(long writeId, boolean revert, boolean persist, Clock serverClock) {
        final long writeId2 = writeId;
        final boolean revert2 = revert;
        final boolean persist2 = persist;
        final Clock serverClock2 = serverClock;
        return (List) this.persistenceManager.runInTransaction(new Callable<List<? extends Event>>() {
            public List<? extends Event> call() {
                if (persist2) {
                    SyncTree.this.persistenceManager.removeUserWrite(writeId2);
                }
                UserWriteRecord write = SyncTree.this.pendingWriteTree.getWrite(writeId2);
                boolean needToReevaluate = SyncTree.this.pendingWriteTree.removeWrite(writeId2);
                if (write.isVisible() && !revert2) {
                    Map<String, Object> serverValues = ServerValues.generateServerValues(serverClock2);
                ...
                return SyncTree.this.applyOperationToSyncPoints(new AckUserWrite(write.getPath(), affectedTree, revert2));
            }
        });
    }
    ...
    public /* synthetic */ Node m1786lambda$getServerValue$0$comgooglefirebasedatabasecoreSyncTree(QuerySpec query) throws Exception {
        boolean z;
        Node node;
        Node serverCacheNode;
        Path path = query.getPath();
        Node serverCacheNode2 = null;
        ...
        while (true) {
            ...
            if (currentSyncPoint != null) {
                if (serverCacheNode2 != null) {
                    serverCacheNode = serverCacheNode2;
                } else {
                ...
                serverCacheNode2 = serverCacheNode;
            }
        ...
        if (syncPoint == null) {
            ...
            if (serverCacheNode2 != null) {
                node = serverCacheNode2;
            } else {
                node = syncPoint.getCompleteServerCache(Path.getEmptyPath());
            }
            serverCacheNode2 = node;
        }
        IndexedNode from = IndexedNode.from(serverCacheNode2 != null ? serverCacheNode2 : EmptyNode.Empty(), query.getIndex());
        if (serverCacheNode2 == null) {
            z = false;
        }
        ...
        return (List) this.persistenceManager.runInTransaction(new Callable<List<? extends Event>>() {
            public List<? extends Event> call() {
                ...
                if (syncPoint == null) {
                    ...
                    ImmutableTree unused = SyncTree.this.syncPointTree = SyncTree.this.syncPointTree.set(path, syncPoint);
                } else {
                ...
                SyncTree.this.persistenceManager.setQueryActive(query);
                if (serverCacheNode2 != null) {
                    persistentServerCache = new CacheNode(IndexedNode.from(serverCacheNode2, query.getIndex()), true, false);
                } else {
                    ...
                    if (persistentServerCache.isFullyInitialized()) {
                        ...
                        Node serverCacheNode3 = EmptyNode.Empty();
                        ...
                        while (it.hasNext()) {
                            ...
                            if (!(childSyncPoint == null || (completeCache = childSyncPoint.getCompleteServerCache(Path.getEmptyPath())) == null)) {
                                serverCacheNode3 = serverCacheNode3.updateImmediateChild(child.getKey(), completeCache);
                            }
                        }
                        for (NamedNode child2 : persistentServerCache.getNode()) {
                            if (!serverCacheNode3.hasChild(child2.getName())) {
                                serverCacheNode3 = serverCacheNode3.updateImmediateChild(child2.getName(), child2.getNode());
                            }
                        }
                        persistentServerCache = new CacheNode(IndexedNode.from(serverCacheNode3, query.getIndex()), false, false);
                    }
                }
    ...
    public List<Event> removeEventRegistration(EventRegistration eventRegistration, boolean skipDedup) {
        return removeEventRegistration(eventRegistration.getQuerySpec(), eventRegistration, (DatabaseError) null, skipDedup);
    }

    public List<Event> removeAllEventRegistrations(QuerySpec query, DatabaseError error) {
        return removeEventRegistration(query, (EventRegistration) null, error, false);
    }

    private List<Event> removeEventRegistration(QuerySpec query, EventRegistration eventRegistration, DatabaseError cancelError, boolean skipDedup) {
        final QuerySpec query2 = query;
        final EventRegistration eventRegistration2 = eventRegistration;
        final DatabaseError cancelError2 = cancelError;
        final boolean skipDedup2 = skipDedup;
        return (List) this.persistenceManager.runInTransaction(new Callable<List<Event>>() {
            public List<Event> call() {
                Path path = query2.getPath();
                ...
                if (maybeSyncPoint == null) {
                ...
                } else if (query2.isDefault() || maybeSyncPoint.viewExistsForQuery(query2)) {
                    Pair<List<QuerySpec>, List<Event>> removedAndEvents = maybeSyncPoint.removeEventRegistration(query2, eventRegistration2, cancelError2);
                    if (maybeSyncPoint.isEmpty()) {
                        ImmutableTree unused = SyncTree.this.syncPointTree = SyncTree.this.syncPointTree.remove(path);
                    }
                    ...
                    for (QuerySpec queryRemoved : removed) {
                        SyncTree.this.persistenceManager.setQueryInactive(query2);
                    ...
                    if (!covered && !removed.isEmpty() && cancelError2 == null) {
                        if (removingDefault) {
                            SyncTree.this.listenProvider.stopListening(SyncTree.this.queryForListening(query2), (Tag) null);
                        } else {
    ...
    public List<Event> applyOperationDescendantsHelper(Operation operation, ImmutableTree<SyncPoint> syncPointTree2, Node serverCache, WriteTreeRef writesCache) {
        ...
        final WriteTreeRef writesCache2 = writesCache;
        syncPointTree2.getChildren().inOrderTraversal(new LLRBNode.NodeVisitor<ChildKey, ImmutableTree<SyncPoint>>() {
            public void visitEntry(ChildKey key, ImmutableTree<SyncPoint> childTree) {
                ...
                WriteTreeRef childWritesCache = writesCache2.child(key);
        ...
        if (syncPoint != null) {
            events.addAll(syncPoint.applyOperation(operation2, writesCache2, resolvedServerCache));
        }
        return events;
    }
