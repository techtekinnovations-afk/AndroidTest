...
public class AsyncQueue {
    ...
    public static <TResult> Task<TResult> callTask(Executor executor2, Callable<Task<TResult>> task) {
        ...
        executor2.execute(new AsyncQueue$$ExternalSyntheticLambda6(task, executor2, tcs));
        return tcs.getTask();
    }

    static /* synthetic */ void lambda$callTask$1(Callable task, Executor executor2, TaskCompletionSource tcs) {
        try {
            ((Task) task.call()).continueWith(executor2, new AsyncQueue$$ExternalSyntheticLambda2(tcs));
        } catch (Exception e) {
            tcs.setException(e);
        } catch (Throwable t) {
    ...
    private class SynchronizedShutdownAwareExecutor implements Executor {
        ...
        SynchronizedShutdownAwareExecutor() {
            ...
            this.thread.setUncaughtExceptionHandler(new AsyncQueue$SynchronizedShutdownAwareExecutor$$ExternalSyntheticLambda1(this));
            this.internalExecutor = new ScheduledThreadPoolExecutor(1, threadFactory, AsyncQueue.this) {
            ...
            this.internalExecutor.setKeepAliveTime(3, TimeUnit.SECONDS);
        }
        ...
        public /* synthetic */ void m1926lambda$new$0$comgooglefirebasefirestoreutilAsyncQueue$SynchronizedShutdownAwareExecutor(Thread crashingThread, Throwable throwable) {
        ...
        public <T> Task<T> executeAndReportResult(Callable<T> task) {
            ...
            try {
                execute(new AsyncQueue$SynchronizedShutdownAwareExecutor$$ExternalSyntheticLambda2(completionSource, task));
            } catch (RejectedExecutionException e) {
                Logger.warn(AsyncQueue.class.getSimpleName(), "Refused to enqueue task after panic", new Object[0]);
            }
        ...
        public synchronized Task<Void> executeAndInitiateShutdown(Runnable task) {
            ...
            Task<Void> t = executeAndReportResult(new AsyncQueue$SynchronizedShutdownAwareExecutor$$ExternalSyntheticLambda0(task));
    ...
    public Task<Void> enqueue(Runnable task) {
        return enqueue(new AsyncQueue$$ExternalSyntheticLambda4(task));
    }
    ...
    static /* synthetic */ void lambda$panic$3(Throwable t) {
        if (t instanceof OutOfMemoryError) {
            OutOfMemoryError error = new OutOfMemoryError("Firestore (26.0.1) ran out of memory. Check your queries to make sure they are not loading an excessive amount of data.");
            error.initCause(t);
            throw error;
        }
        throw new RuntimeException("Internal error in Cloud Firestore (26.0.1).", t);
    }

    public void runSync(Runnable task) throws InterruptedException {
        Semaphore done = new Semaphore(0);
        Throwable[] t = new Throwable[1];
        enqueueAndForget(new AsyncQueue$$ExternalSyntheticLambda5(task, t, done));
        done.acquire(1);
        if (t[0] != null) {
    ...
    public /* synthetic */ void m1925lambda$runDelayedTasksUntil$6$comgooglefirebasefirestoreutilAsyncQueue(TimerId lastTimerId) {
        ...
        Collections.sort(this.delayedTasks, new AsyncQueue$$ExternalSyntheticLambda0());
