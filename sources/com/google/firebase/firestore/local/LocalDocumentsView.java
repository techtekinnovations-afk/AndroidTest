...
class LocalDocumentsView {
    ...
    /* access modifiers changed from: package-private */
    public Map<DocumentKey, OverlayedDocument> getOverlayedDocuments(Map<DocumentKey, MutableDocument> docs) {
        Map<DocumentKey, Overlay> overlays = new HashMap<>();
        populateOverlays(overlays, docs.keySet());
    ...
    private Map<DocumentKey, OverlayedDocument> computeViews(Map<DocumentKey, MutableDocument> docs, Map<DocumentKey, Overlay> overlays, Set<DocumentKey> existenceStateChanged) {
        ...
        mutatedFields.putAll(recalculateAndSaveOverlays(recalculateDocuments));
    ...
    /* access modifiers changed from: package-private */
    public ImmutableSortedMap<DocumentKey, Document> getDocumentsMatchingQuery(Query query, FieldIndex.IndexOffset offset, QueryContext context) {
    ...
    public ImmutableSortedMap<DocumentKey, Document> getDocumentsMatchingQuery(Query query, FieldIndex.IndexOffset offset) {
        return getDocumentsMatchingQuery(query, offset, (QueryContext) null);
    }
    ...
    private ImmutableSortedMap<DocumentKey, Document> getDocumentsMatchingCollectionGroupQuery(Query query, FieldIndex.IndexOffset offset, QueryContext context) {
        ...
        for (ResourcePath parent : this.indexManager.getCollectionParents(collectionId)) {
            Iterator<Map.Entry<DocumentKey, Document>> it = getDocumentsMatchingCollectionQuery(query.asCollectionQueryAtPath((ResourcePath) parent.append(collectionId)), offset, context).iterator();
            while (it.hasNext()) {
    ...
    public LocalDocumentsResult getNextDocuments(String collectionGroup, FieldIndex.IndexOffset offset, int count) {
        ...
        populateOverlays(overlays, docs.keySet());
        return LocalDocumentsResult.fromOverlayedDocuments(largestBatchId, computeViews(docs, overlays, Collections.emptySet()));
    }
