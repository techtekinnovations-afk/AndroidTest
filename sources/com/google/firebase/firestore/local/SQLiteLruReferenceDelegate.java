...
class SQLiteLruReferenceDelegate implements ReferenceDelegate, LruDelegate {
    ...
    public long getSequenceNumberCount() {
        return this.persistence.getTargetCache().getTargetCount() + ((Long) this.persistence.query("SELECT COUNT(*) FROM (SELECT sequence_number FROM target_documents GROUP BY path HAVING COUNT(*) = 1 AND target_id = 0)").firstValue(new SQLiteLruReferenceDelegate$$ExternalSyntheticLambda2())).longValue();
    }
    ...
    public void forEachOrphanedDocumentSequenceNumber(Consumer<Long> consumer) {
        this.persistence.query("select sequence_number from target_documents group by path having COUNT(*) = 1 AND target_id = 0").forEach(new SQLiteLruReferenceDelegate$$ExternalSyntheticLambda0(consumer));
    }
    ...
    public int removeOrphanedDocuments(long upperBound) {
        ...
        ResourcePath[] startPath = {ResourcePath.EMPTY};
        while (resultsRemaining) {
            resultsRemaining = this.persistence.query("select path from target_documents group by path having COUNT(*) = 1 AND target_id = 0 AND sequence_number <= ? AND path > ? LIMIT ?").binding(Long.valueOf(upperBound), EncodedPath.encode(startPath[0]), 100).forEach(new SQLiteLruReferenceDelegate$$ExternalSyntheticLambda1(this, count, docsToRemove, startPath)) == 100;
        }
    ...
    public /* synthetic */ void m1871lambda$removeOrphanedDocuments$2$comgooglefirebasefirestorelocalSQLiteLruReferenceDelegate(int[] count, List docsToRemove, ResourcePath[] startPath, Cursor row) {
    ...
    private void writeSentinel(DocumentKey key) {
        this.persistence.execute("INSERT OR REPLACE INTO target_documents (target_id, path, sequence_number) VALUES (0, ?, ?)", EncodedPath.encode(key.getPath()), Long.valueOf(getCurrentSequenceNumber()));
    }
