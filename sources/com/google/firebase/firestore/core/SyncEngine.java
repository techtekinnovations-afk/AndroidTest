...
public class SyncEngine implements RemoteStore.RemoteStoreCallback {
    ...
    public int listen(Query query, boolean shouldListenToRemote) {
        ...
        this.syncEngineListener.onViewSnapshots(Collections.singletonList(initializeViewAndComputeSnapshot(query, targetData.getTargetId(), targetData.getResumeToken())));
    ...
    private ViewSnapshot initializeViewAndComputeSnapshot(Query query, int targetId, ByteString resumeToken) {
        ...
        if (this.queriesByTarget.get(Integer.valueOf(targetId)) != null) {
            currentTargetSyncState = this.queryViewsByQuery.get((Query) this.queriesByTarget.get(Integer.valueOf(targetId)).get(0)).getView().getSyncState();
        }
        if (currentTargetSyncState == ViewSnapshot.SyncState.SYNCED) {
            z = true;
        }
        TargetChange synthesizedCurrentChange = TargetChange.createSynthesizedTargetChangeForCurrentChange(z, resumeToken);
    ...
    public void stopListening(Query query, boolean shouldUnlistenToRemote) {
    ...
    public void handleRejectedListen(int targetId, Status error) {
        ...
        if (limboKey != null) {
            ...
            handleRemoteEvent(new RemoteEvent(SnapshotVersion.NONE, Collections.emptyMap(), Collections.emptyMap(), Collections.singletonMap(limboKey, MutableDocument.newNoDocument(limboKey, SnapshotVersion.NONE)), Collections.singleton(limboKey)));
            return;
        }
        this.localStore.releaseTarget(targetId);
    ...
    private void notifyUser(int r5, io.grpc.Status r6) {
        /*
            r4 = this;
