        ...
        if (key == null) {
            ...
            if (obj2 instanceof String) {
                try {
                    ...
                    throw new IllegalArgumentException(String.format("string value '%s' for key '%s' cannot be parsed as a float", new Object[]{obj2, key}));
                }
            } else {
                throw new IllegalArgumentException(String.format("value %s for key '%s' is not a float", new Object[]{obj2, key}));
            }
        }
    }
    ...
    public static List<Map<String, ?>> checkObjectList(List<?> rawList) {
        ...
        while (i < rawList.size()) {
            if (rawList.get(i) instanceof Map) {
                ...
                throw new ClassCastException(String.format(Locale.US, "value %s for idx %d in %s is not object", new Object[]{rawList.get(i), Integer.valueOf(i), rawList}));
            }
        }
        return rawList;
    }
    ...
    public static List<String> checkStringList(List<?> rawList) {
        ...
        while (i < rawList.size()) {
            if (rawList.get(i) instanceof String) {
                ...
                throw new ClassCastException(String.format(Locale.US, "value '%s' for idx %d in '%s' is not string", new Object[]{rawList.get(i), Integer.valueOf(i), rawList}));
            }
        }
        return rawList;
    }
