...
public class ApplicationThreadDeframer implements Deframer {
    ...
    ApplicationThreadDeframer(MessageDeframer.Listener listener, TransportExecutor transportExecutor, MessageDeframer deframer2) {
        this.storedListener = new SquelchLateMessagesAvailableDeframerListener((MessageDeframer.Listener) Preconditions.checkNotNull(listener, ServiceSpecificExtraArgs.CastExtraArgs.LISTENER));
        this.appListener = new ApplicationThreadDeframerListener(this.storedListener, transportExecutor);
        deframer2.setListener(this.appListener);
