            ...
            while (true) {
                ...
                if (!ref.shutdown.get()) {
                    ...
                    if (ManagedChannelOrphanWrapper.logger.isLoggable(level)) {
                        LogRecord lr = new LogRecord(level, "*~*~*~ Previous channel {0} was garbage collected without being shut down! ~*~*~*" + System.getProperty("line.separator") + "    Make sure to call shutdown()/shutdownNow()");
