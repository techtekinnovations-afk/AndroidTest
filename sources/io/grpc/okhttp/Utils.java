...
class Utils {
    ...
    static ConnectionSpec convertSpec(com.squareup.okhttp.ConnectionSpec spec) {
        ...
        io.grpc.okhttp.internal.CipherSuite[] cipherSuites = new io.grpc.okhttp.internal.CipherSuite[cipherSuiteList.size()];
        for (int i2 = 0; i2 < cipherSuites.length; i2++) {
            cipherSuites[i2] = io.grpc.okhttp.internal.CipherSuite.valueOf(cipherSuiteList.get(i2).name());
        }
