...
public final class Http2 implements Variant {
    ...
    static final class Reader implements FrameReader {
        ...
        private void readData(FrameReader.Handler handler, int paddedLength, byte flags, int streamId) throws IOException {
            short padding = 0;
            ...
            if (!gzipped) {
                ...
                handler.data(inFinished, streamId, this.source, Http2.lengthWithoutPadding(paddedLength, flags, padding), paddedLength);
    ...
    public static IllegalArgumentException illegalArgument(String message, Object... args) {
        throw new IllegalArgumentException(String.format(Locale.US, message, args));
    }
    ...
    static final class FrameLogger {
        ...
        static String formatHeader(boolean inbound, int streamId, int length, byte type, byte flags) {
            return String.format(Locale.US, "%s 0x%08x %5d %-13s %s", new Object[]{inbound ? "<<" : ">>", Integer.valueOf(streamId), Integer.valueOf(length), type < TYPES.length ? TYPES[type] : String.format("0x%02x", new Object[]{Byte.valueOf(type)}), formatFlags(type, flags)});
        }
    ...
    public static int readMedium(BufferedSource source) throws IOException {
        return ((source.readByte() & 255) << Ascii.DLE) | ((source.readByte() & 255) << 8) | (source.readByte() & 255);
    }
