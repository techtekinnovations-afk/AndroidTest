...
public final class Metadata {
    ...
    public <T> boolean remove(Key<T> key, T value) {
        ...
        for (int i = 0; i < this.size; i++) {
            if (bytesEqual(key.asciiName(), name(i)) && value.equals(valueAsT(i, key))) {
                int readIdx = (i + 1) * 2;
                System.arraycopy(this.namesAndValues, readIdx, this.namesAndValues, i * 2, len() - readIdx);
                this.size--;
    ...
    public <T> Iterable<T> removeAll(Key<T> key) {
        ...
        for (int readIdx = 0; readIdx < this.size; readIdx++) {
            if (bytesEqual(key.asciiName(), name(readIdx))) {
                List<T> ret2 = ret != null ? ret : new ArrayList<>();
                ret2.add(valueAsT(readIdx, key));
                ret = ret2;
            } else {
    ...
    public static abstract class Key<T> {
        ...
        public static <T> Key<T> of(String name2, BinaryMarshaller<T> marshaller2) {
            return new BinaryKey(name2, marshaller2);
        }

        public static <T> Key<T> of(String name2, BinaryStreamMarshaller<T> marshaller2) {
            return new LazyStreamBinaryKey(name2, marshaller2);
        }

        public static <T> Key<T> of(String name2, AsciiMarshaller<T> marshaller2) {
            return of(name2, false, marshaller2);
        }

        static <T> Key<T> of(String name2, boolean pseudo, AsciiMarshaller<T> marshaller2) {
            return new AsciiKey(name2, pseudo, marshaller2);
        }

        static <T> Key<T> of(String name2, boolean pseudo, TrustedAsciiMarshaller<T> marshaller2) {
            return new TrustedAsciiKey(name2, pseudo, marshaller2);
        }

        private static BitSet generateValidTChars() {
            BitSet valid = new BitSet(WorkQueueKt.MASK);
            valid.set(45);
            valid.set(95);
            valid.set(46);
        ...
        private Key(String name2, boolean pseudo, Object marshaller2) {
            this.originalName = (String) Preconditions.checkNotNull(name2, "name");
    ...
    private static class BinaryKey<T> extends Key<T> {
        ...
        public byte[] toBytes(T value) {
            return (byte[]) Preconditions.checkNotNull(this.marshaller.toBytes(value), "null marshaller.toBytes()");
        }
    ...
    private static class LazyStreamBinaryKey<T> extends Key<T> {
        ...
        /* access modifiers changed from: package-private */
        public byte[] toBytes(T value) {
            return Metadata.streamToBytes((InputStream) Preconditions.checkNotNull(this.marshaller.toStream(value), "null marshaller.toStream()"));
        }
    ...
    static final class LazyValue<T> {
        ...
        public InputStream toStream() {
            return (InputStream) Preconditions.checkNotNull(this.marshaller.toStream(this.value), "null marshaller.toStream()");
        }
    ...
    private static class AsciiKey<T> extends Key<T> {
        ...
        /* access modifiers changed from: package-private */
        public byte[] toBytes(T value) {
            return ((String) Preconditions.checkNotNull(this.marshaller.toAsciiString(value), "null marshaller.toAsciiString()")).getBytes(Charsets.US_ASCII);
        }
    ...
    private static final class TrustedAsciiKey<T> extends Key<T> {
        ...
        /* access modifiers changed from: package-private */
        public byte[] toBytes(T value) {
            return (byte[]) Preconditions.checkNotNull(this.marshaller.toAsciiString(value), "null marshaller.toAsciiString()");
        }
