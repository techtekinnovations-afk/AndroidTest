...
public final class RealBufferedSource implements BufferedSource {
    ...
    public int select(Options options) {
        ...
        if (!this.closed) {
            do {
                int index$iv = Buffer.selectPrefix(this.bufferField, options, true);
                switch (index$iv) {
                    case -2:
                        break;
                    case -1:
    ...
    public String readUtf8Line() {
        ...
        if (newline$iv != -1) {
            return Buffer.readUtf8Line(this.bufferField, newline$iv);
        }
        if (this.bufferField.size() != 0) {
    ...
    public String readUtf8LineStrict(long limit) {
        long j = limit;
        if (j >= 0) {
            long scanLength$iv = j == Long.MAX_VALUE ? Long.MAX_VALUE : j + 1;
            long newline$iv = indexOf((byte) 10, 0, scanLength$iv);
            if (newline$iv != -1) {
                return Buffer.readUtf8Line(this.bufferField, newline$iv);
            }
            if (scanLength$iv < Long.MAX_VALUE && request(scanLength$iv) && this.bufferField.getByte(scanLength$iv - 1) == 13 && request(scanLength$iv + 1) && this.bufferField.getByte(scanLength$iv) == 10) {
                return Buffer.readUtf8Line(this.bufferField, scanLength$iv);
            }
            Buffer data$iv = new Buffer();
            this.bufferField.copyTo(data$iv, 0, Math.min((long) 32, this.bufferField.size()));
            throw new EOFException("\\n not found: limit=" + Math.min(this.bufferField.size(), j) + " content=" + data$iv.readByteString().hex() + Typography.ellipsis);
        }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:14:0x0033, code lost:
        r3 = new java.lang.StringBuilder().append("Expected a digit or '-' but was 0x");
        r7 = java.lang.Integer.toString(r6, kotlin.text.CharsKt.checkRadix(kotlin.text.CharsKt.checkRadix(16)));
        kotlin.jvm.internal.Intrinsics.checkNotNullExpressionValue(r7, "toString(this, checkRadix(radix))");
     */
    ...
    public long indexOf(byte b, long fromIndex, long toIndex) {
        ...
        if (!this.closed) {
            boolean z = false;
            if (0 <= fromIndex$iv && fromIndex$iv <= toIndex) {
                z = true;
            }
            if (z) {
                long fromIndex$iv2 = fromIndex$iv;
                while (fromIndex$iv2 < toIndex) {
                    long j = toIndex;
                    long result$iv = this.bufferField.indexOf(b, fromIndex$iv2, j);
                    if (result$iv != -1) {
                        return result$iv;
                    }
                    long lastBufferSize$iv = this.bufferField.size();
                    if (lastBufferSize$iv >= j || this.source.read(this.bufferField, 8192) == -1) {
                        return -1;
                    }
            ...
            throw new IllegalArgumentException(("fromIndex=" + fromIndex$iv + " toIndex=" + toIndex).toString());
        }
        long j3 = toIndex;
        throw new IllegalStateException("closed".toString());
    }
    ...
    public long indexOf(ByteString bytes, long fromIndex) {
        ...
        if (!this.closed) {
            while (true) {
                ...
                fromIndex$iv = Math.max(fromIndex$iv, (lastBufferSize$iv - ((long) byteString.size())) + 1);
            }
        } else {
            throw new IllegalStateException("closed".toString());
        }
