...
final class UIntProgressionIterator implements Iterator<UInt>, KMappedMarker {
    ...
    private UIntProgressionIterator(int first, int last, int step2) {
        ...
        boolean z = true;
        if (step2 <= 0 ? Integer.compare(first ^ Integer.MIN_VALUE, last ^ Integer.MIN_VALUE) < 0 : Integer.compare(first ^ Integer.MIN_VALUE, last ^ Integer.MIN_VALUE) > 0) {
            z = false;
        }
        this.hasNext = z;
        this.step = UInt.m101constructorimpl(step2);
        this.next = this.hasNext ? first : this.finalElement;
    }

    public /* bridge */ /* synthetic */ Object next() {
        return UInt.m95boximpl(m1228nextpVg5ArA());
    }
    ...
    public int m1228nextpVg5ArA() {
        int value = this.next;
        if (value != this.finalElement) {
            this.next = UInt.m101constructorimpl(this.next + this.step);
