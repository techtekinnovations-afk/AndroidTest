...
final class EncodeOutputStream extends OutputStream {
    ...
    public EncodeOutputStream(OutputStream output2, Base64 base642) {
        ...
        if (offset < 0 || length < 0 || offset + length > source.length) {
            ...
            if (this.byteBufferLength < 3) {
                ...
                while (startIndex + 3 <= endIndex) {
                    int groupsToEncode = Math.min((this.base64.isMimeScheme$kotlin_stdlib() ? this.lineLength : this.symbolBuffer.length) / 4, (endIndex - startIndex) / 3);
                    int bytesToEncode = groupsToEncode * 3;
                    if (encodeIntoOutput(source, startIndex, startIndex + bytesToEncode) == groupsToEncode * 4) {
